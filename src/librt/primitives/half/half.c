 ...
 */
C_DECL int
rt_hlf_plot(struct bu_list *vhead, struct rt_db_internal *ip, const struct bg_tess_tol *UNUSED(ttol), const struct bn_tol *UNUSED(tol), const struct bv_view_info *UNUSED(info))
{
    struct rt_half_internal *hip;
    ...
    RT_ADD_VLIST(vlfree, vhead, x_1, RT_VLIST_LINE_MOVE);	/* the cross */
    RT_ADD_VLIST(vlfree, vhead, x_2, RT_VLIST_LINE_DRAW);
    RT_ADD_VLIST(vlfree, vhead, y_1, RT_VLIST_LINE_MOVE);
    RT_ADD_VLIST(vlfree, vhead, y_2, RT_VLIST_LINE_DRAW);
    RT_ADD_VLIST(vlfree, vhead, x_2, RT_VLIST_LINE_DRAW);	/* the box */
    RT_ADD_VLIST(vlfree, vhead, y_1, RT_VLIST_LINE_DRAW);
    RT_ADD_VLIST(vlfree, vhead, x_1, RT_VLIST_LINE_DRAW);
    RT_ADD_VLIST(vlfree, vhead, y_2, RT_VLIST_LINE_DRAW);

    VSCALE(tip, hip->eqn, 500);
    VADD2(tip, cent, tip);
    RT_ADD_VLIST(vlfree, vhead, cent, RT_VLIST_LINE_MOVE);
    RT_ADD_VLIST(vlfree, vhead, tip, RT_VLIST_LINE_DRAW);
    return 0;
}
