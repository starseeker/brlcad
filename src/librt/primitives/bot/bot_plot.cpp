...
plot_node(const vdsNode *node, void *udata)
{
	...
	if (nd->ok && !primitive_lod_line_set_append(nd->realization,
		t->proxies[2]->coord, RT_PRIMITIVE_LINE_MOVE))
	    nd->ok = 0;
	if (nd->ok && !primitive_lod_line_set_append(nd->realization,
		t->proxies[0]->coord, RT_PRIMITIVE_LINE_DRAW))
	    nd->ok = 0;
	if (nd->ok && !primitive_lod_line_set_append(nd->realization,
		t->proxies[1]->coord, RT_PRIMITIVE_LINE_DRAW))
	    nd->ok = 0;
	if (nd->ok && !primitive_lod_line_set_append(nd->realization,
		t->proxies[2]->coord, RT_PRIMITIVE_LINE_DRAW))
	    nd->ok = 0;
	t = t->next;
    }
}
...
avg_sample_spacing(const struct bv_view_info *info)
{
    ...
    fastf_t view_aspect = (fastf_t)info->width / info->height;
    fastf_t x_size = info->size;
    fastf_t y_size = x_size / view_aspect;
    fastf_t avg_view_size = (x_size + y_size) / 2.0;
    fastf_t avg_view_samples = (info->width + info->height) / 2.0;
...
#define BOT_BBOX_ARB_FACE(valp, a, b, c, d)             \
    RT_ADD_VLIST(vlfree, vhead, valp[a], RT_VLIST_LINE_MOVE);   \
    RT_ADD_VLIST(vlfree, vhead, valp[b], RT_VLIST_LINE_DRAW);   \
    RT_ADD_VLIST(vlfree, vhead, valp[c], RT_VLIST_LINE_DRAW);   \
    RT_ADD_VLIST(vlfree, vhead, valp[d], RT_VLIST_LINE_DRAW);
...
rt_bot_plot(struct bu_list *vhead, struct rt_db_internal *ip, const struct bg_tess_tol *UNUSED(ttol), const struct bn_tol *UNUSED(tol), const struct bv_view_info *info)
{
	...
	for (i = 0; i < bot_ip->num_faces; i++) {
	    ...
	    RT_ADD_VLIST(vlfree, vhead, &bot_ip->vertices[bot_ip->faces[i*3+0]*3], RT_VLIST_LINE_MOVE);
	    RT_ADD_VLIST(vlfree, vhead, &bot_ip->vertices[bot_ip->faces[i*3+1]*3], RT_VLIST_LINE_DRAW);
	    RT_ADD_VLIST(vlfree, vhead, &bot_ip->vertices[bot_ip->faces[i*3+2]*3], RT_VLIST_LINE_DRAW);
	    RT_ADD_VLIST(vlfree, vhead, &bot_ip->vertices[bot_ip->faces[i*3+0]*3], RT_VLIST_LINE_DRAW);
	}
    } else {
