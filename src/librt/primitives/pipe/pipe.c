...
{
    struct wdb_pipe_pnt *prevpt, *curpt, *nextpt;
    fastf_t dot, rad_between_segments, supplementary_angle;
    vect_t cur_to_prev, cur_to_next;

    curpt = seg->cur;
    prevpt = BU_LIST_PREV(wdb_pipe_pnt, &curpt->l);
    nextpt = BU_LIST_NEXT(wdb_pipe_pnt, &curpt->l);
...
pipe_seg_first(struct rt_pipe_internal *pipeobj)
{
    ...
    pt_1 = BU_LIST_FIRST(wdb_pipe_pnt, seghead);
    pt_2 = BU_LIST_NEXT(wdb_pipe_pnt, &pt_1->l);
...
{
    seg->cur = BU_LIST_NEXT(wdb_pipe_pnt, &seg->cur->l);
}
...
{
    struct wdb_pipe_pnt *nextpt;

    nextpt = BU_LIST_NEXT(wdb_pipe_pnt, &seg->cur->l);
...
void
rt_pipe_pnt_print(const struct wdb_pipe_pnt *pipe_pnt, double mm2local)
{
    point_t p1;

    bu_log("Pipe Vertex:\n");
    VSCALE(p1, pipe_pnt->pp_coord, mm2local);
    bu_log("\tat (%g %g %g)\n", V3ARGS(p1));
    bu_log("\tbend radius = %g\n", pipe_pnt->pp_bendradius * mm2local);
    if (pipe_pnt->pp_id > 0.0) {
	bu_log("\tod=%g, id=%g\n",
	       pipe_pnt->pp_od * mm2local,
	       pipe_pnt->pp_id * mm2local);
    } else {
	bu_log("\tod=%g\n", pipe_pnt->pp_od * mm2local);
    }
}
    ...
    double mm2local)
{
    ...
    pipe_pnt = BU_LIST_FIRST(wdb_pipe_pnt, &pint->pipe_segs_head);
    while (++seg_count != seg_no && BU_LIST_NOT_HEAD(&pipe_pnt->l, &pint->pipe_segs_head)) {
	pipe_pnt = BU_LIST_NEXT(wdb_pipe_pnt, &pipe_pnt->l);
    }
...
static void
pipe_line_sink_append(struct pipe_line_sink *sink, const point_t p, int command)
{
    if (!sink || !sink->ok)
	return;
    ...
    int num_segments)
{
    ...
    if (sink->realization) {
	if (!primitive_lod_append_ellipse(sink->realization, center, axis_a,
		axis_b, num_segments))
	    sink->ok = 0;
	return;
    }
    ...
    plot_ellipse(sink->vlfree, sink->vhead, center, axis_a, axis_b,
	    num_segments);
}
    ...
    int num_segments)
{
    ...
    pipe_line_sink_append_ellipse(sink, circle->center, axis_a, axis_b,
	    num_segments);
}
    ...
    int num_lines)
{
	...
	pipe_line_sink_append(sink, pt, RT_PRIMITIVE_LINE_MOVE);

	ellipse_point_at_radian(pt, end->center, end_a, end_b, radian);
	pipe_line_sink_append(sink, pt, RT_PRIMITIVE_LINE_DRAW);
    ...
    int num_connections)
{
    ...
    draw_pipe_parallel_circle_connections(sink, &start_circle, &end_circle,
					  num_connections);
...
{
    struct wdb_pipe_pnt startpt, endpt;

    endpt = *seg->cur;
    startpt = *BU_LIST_PREV(wdb_pipe_pnt, &seg->cur->l);
    VMOVE(startpt.pp_coord, seg->last_drawn);

    draw_pipe_connect_points_linearly(sink, &startpt, &endpt, &seg->orient,
				      seg->connecting_arcs);
    ...
    int segs_per_arc)
{
	...
	draw_pipe_arc(sink, arc_circle, arc_end, segs_per_arc);
...
static void
draw_pipe_circular_seg(struct pipe_line_sink *sink, struct pipe_segment *seg)
{
    ...
    seg->orient = draw_pipe_connect_circular_segs(sink, &bend,
...
static void
draw_pipe_end(struct pipe_line_sink *sink, struct pipe_segment *seg)
{
    ...
    draw_pipe_circle(sink, &pipe_circle, PIPE_CIRCLE_SEGS);
    ...
    fastf_t point_spacing)
{
    ...
    draw_pipe_circle(sink, &pipe_circle, num_segments);
...
rt_pipe_lod_line_set(struct rt_primitive_lod_realization *realization, struct rt_db_internal *ip, const struct bn_tol *UNUSED(tol), const struct rt_view_info *v, fastf_t s_size)
{
    ...
    draw_pipe_end_adaptive(&sink, cur_seg, point_spacing);
    pipe_seg_advance(cur_seg);

    while (!pipe_seg_is_last(cur_seg)) {
	if (pipe_seg_is_bend(cur_seg)) {
	    draw_pipe_circular_seg_adaptive(&sink, cur_seg, point_spacing);
	} else {
	    draw_pipe_linear_seg(&sink, cur_seg);
	}
...
rt_pipe_lod_realize(struct rt_primitive_lod_realization *realization, struct rt_db_internal *ip, const struct bn_tol *tol, const struct rt_view_info *v, fastf_t s_size)
{
    ...
    int ret = rt_pipe_lod_line_set(realization, ip, tol, v, s_size);
    if (ret < 0)
	return ret;
...
static int
rt_pipe_standard_line_set(struct pipe_line_sink *sink, struct rt_pipe_internal *pip)
{
    struct pipe_segment *cur_seg;
    ...
    draw_pipe_end(sink, cur_seg);
    pipe_seg_advance(cur_seg);

    while (!pipe_seg_is_last(cur_seg)) {
	if (pipe_seg_is_bend(cur_seg)) {
	    draw_pipe_circular_seg(sink, cur_seg);
	} else {
	    draw_pipe_linear_seg(sink, cur_seg);
	}
...
int
rt_pipe_wireframe_line_set(struct rt_primitive_lod_realization *realization,
			   struct rt_db_internal *ip)
{
    struct rt_pipe_internal *pip;
    ...
    int ret = rt_pipe_standard_line_set(&sink, pip);
    if (ret < 0)
	return ret;
    return primitive_lod_line_set_finish(realization) ? ret : -1;
}
    ...
    const struct rt_view_info *UNUSED(info))
{
    ...
    return rt_pipe_standard_line_set(&sink, pip);
}
    ...
    struct bu_list *vlfree)
{
    ...
    next = BU_LIST_NEXT(wdb_pipe_pnt, &pipe_pnt->l);

    VSUB2(n, pipe_pnt->pp_coord, next->pp_coord);
    VUNITIZE(n);
    bn_vec_ortho(r1, n);
    VCROSS(r2, n, r1);

    orad = pipe_pnt->pp_od / 2.0;
    irad = pipe_pnt->pp_id / 2.0;
	...
	VJOIN2(pt, pipe_pnt->pp_coord, x, r1, y, r2);
    ...
    struct bu_list *vlfree)
{
    struct vertex **new_outer_loop = NULL;
    struct vertex **new_inner_loop = NULL;
    struct bg_vert_tree *vertex_tree = NULL;
    ...
    BN_CK_TOL(tol);
    BG_CK_TESS_TOL(ttol);
    vertex_tree = bg_vert_tree_create();
	...
	j = bg_vert_tree_add( vertex_tree,vg->coord[X], vg->coord[Y], vg->coord[Z], tol->dist_sq);
	vertex_array[j] = v;
    }
	...
	for (i = 0; i < arc_segs; i++) {
	    ...
	    k = bg_vert_tree_add( vertex_tree,pt[X], pt[Y], pt[Z], tol->dist_sq);
	...
	*outer_loop = new_outer_loop;
    ...
    bg_vert_tree_destroy(vertex_tree);
    ...
    struct bu_list *vlfree)
{
    ...
    if (pipe_pnt->pp_od <= tol->dist) {
	return;
    }

    if (NEAR_EQUAL(pipe_pnt->pp_od, pipe_pnt->pp_id, tol->dist)) {
	return;
    }
...
{
    struct exported_pipe_pnt *exp_pipe_pnt;
    struct wdb_pipe_pnt *ptp;
    struct wdb_pipe_pnt tmp;
    struct rt_pipe_internal *pip;
    union record *rp;
    ...
    for (exp_pipe_pnt = &rp->pwr.pwr_data[pip->pipe_count - 1]; exp_pipe_pnt >= &rp->pwr.pwr_data[0]; exp_pipe_pnt--) {
	bu_cv_ntohd((unsigned char *)&scan[0], exp_pipe_pnt->epp_id, 1);
	tmp.pp_id = scan[0]; /* convert double to fastf_t */

	bu_cv_ntohd((unsigned char *)&scan[1], exp_pipe_pnt->epp_od, 1);
	tmp.pp_od = scan[1]; /* convert double to fastf_t */

	bu_cv_ntohd((unsigned char *)&scan[2], exp_pipe_pnt->epp_bendradius, 1);
	tmp.pp_bendradius = scan[2]; /* convert double to fastf_t */

	bu_cv_ntohd((unsigned char *)scan, exp_pipe_pnt->epp_coord, ELEMENTS_PER_VECT);
    ...
    const struct db_i *dbip)
{
    ...
    count = 0;
    for (BU_LIST_FOR(ppt, wdb_pipe_pnt, headp)) {
	count++;
    }
	...
	(count - 1) * sizeof(struct exported_pipe_pnt);
    ...
    *(uint32_t *)rec->pwr.pwr_pt_count = htonl(count);
    ...
    for (BU_LIST_FOR(ppt, wdb_pipe_pnt, headp), epp++) {
...
rt_pipe_export5(struct bu_external *ep, const struct rt_db_internal *ip, double local2mm, const struct db_i *dbip)
{
    ...
    *(uint32_t *)ep->ext_buf = htonl(pipe_count);
    ...
    /* Convert the pipe segments to external form */
    for (BU_LIST_FOR(ppt, wdb_pipe_pnt, headp), i += 6) {
	/* Convert from user units to mm */
    ...
    double mm2local)
{
    ...
    for (BU_LIST_FOR(ptp, wdb_pipe_pnt, &pip->pipe_segs_head)) {
	sprintf(buf, "\t%d ", segno++);
	bu_vls_strcat(str, buf);
...
rt_pipe_ifree(struct rt_db_internal *ip)
{
    ...
    if (pip->pipe_segs_head.magic != 0) {
	while (BU_LIST_WHILE(ptp, wdb_pipe_pnt, &pip->pipe_segs_head)) {
	    BU_LIST_DEQUEUE(&(ptp->l));
	    bu_free((char *)ptp, "wdb_pipe_pnt");
	}
    }
    ...
    const char **argv)
{
	...
	if (seg_no == num_segs) {
	    struct wdb_pipe_pnt *new_pt;

	    BU_ALLOC(new_pt, struct wdb_pipe_pnt);
	    if (num_segs > 0) {
