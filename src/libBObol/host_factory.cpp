...
struct bobol_host_factory_token {
    bobol_host_factory_token(void) : references(0), registered(false)
    {
...
static std::vector<bobol_host_factory_token_t *> registry;

static size_t
factory_minimum_size(void)
{
    return offsetof(struct bobol_host_factory, resize) +
	sizeof(((struct bobol_host_factory *)0)->resize);
}

static struct bobol_host_desc
sanitize_desc(const struct bobol_host_desc *desc)
{
    struct bobol_host_desc actual;
    std::memset(&actual, 0, sizeof(actual));
    actual.struct_size = sizeof(actual);
    actual.mode = BOBOL_HOST_MODE_HEADLESS;
    actual.width = 1;
    actual.height = 1;
	...
	actual.device_pixel_ratio = 1.0;
    if (actual.vsync < BOBOL_HOST_VSYNC_AUTO ||
	actual.vsync > BOBOL_HOST_VSYNC_ON)
	actual.vsync = BOBOL_HOST_VSYNC_AUTO;
    return actual;
}

static bool
factory_compatible(const bobol_host_factory_token_t *token,
...
extern "C" bobol_host_factory_token_t *
bobol_host_factory_register(const struct bobol_host_factory *factory)
{
    if (!factory || factory->abi_version != BOBOL_HOST_FACTORY_ABI_VERSION ||
    ...
    if ((factory->capabilities & BOBOL_HOST_CAP_READBACK) &&
	(!factory->capture || factory->struct_size <
	 offsetof(struct bobol_host_factory, capture) +
	 sizeof(factory->capture)))
	return NULL;

    std::lock_guard<std::mutex> lock(registry_mutex);
    for (const bobol_host_factory_token_t *entry : registry) {
	if (entry->name == factory->name)
	    return NULL;
    }

    bobol_host_factory_token_t *token =
	new (std::nothrow) bobol_host_factory_token_t;
    if (!token)
	return NULL;
...
bobol_host_factory_unregister(bobol_host_factory_token_t *token)
{
    ...
    std::lock_guard<std::mutex> lock(registry_mutex);
    std::vector<bobol_host_factory_token_t *>::iterator it =
...
extern "C" size_t
bobol_host_factory_registry_count(void)
{
    std::lock_guard<std::mutex> lock(registry_mutex);
...
extern "C" size_t
bobol_host_factory_registry_name(size_t index, char *name, size_t name_size)
{
    std::lock_guard<std::mutex> lock(registry_mutex);
...
extern "C" uint64_t
bobol_host_factory_registry_capabilities(size_t index)
{
    std::lock_guard<std::mutex> lock(registry_mutex);
...
extern "C" const char *
bobol_host_factory_name(const bobol_host_factory_token_t *token)
{
    return token ? token->name.c_str() : NULL;
}

extern "C" uint64_t
bobol_host_factory_capabilities(const bobol_host_factory_token_t *token)
{
...
bobol_host_factory_acquire(const char *name,
	const struct bobol_host_desc *desc)
{
    const struct bobol_host_desc actual = sanitize_desc(desc);
    std::vector<bobol_host_factory_token_t *> candidates;
    {
	std::lock_guard<std::mutex> lock(registry_mutex);
	for (bobol_host_factory_token_t *entry : registry) {
	...
	[](const bobol_host_factory_token_t *a,
	   const bobol_host_factory_token_t *b) {
	    if (a->factory.priority != b->factory.priority)
		return a->factory.priority > b->factory.priority;
	    return a->name < b->name;
	});

    bobol_host_factory_token_t *selected = NULL;
    for (bobol_host_factory_token_t *candidate : candidates) {
	...
	if (candidate != selected)
	    bobol_host_factory_release(candidate);
    }
    return selected;
}

extern "C" void
bobol_host_factory_release(bobol_host_factory_token_t *token)
{
    if (!token)
	return;
    std::lock_guard<std::mutex> lock(registry_mutex);
...
extern "C" int
bobol_host_factory_instance_create(bobol_host_factory_token_t *token,
	const struct bobol_host_desc *desc, void *controller, void **instance)
{
    if (instance)
	*instance = NULL;
    ...
    if (!created)
	return 0;
    if (!bobol_host_factory_instance_bind_controller(token, created,
	    controller)) {
	...
	(void)bobol_host_factory_instance_bind_controller(token, created,
	    NULL);
	token->factory.destroy(created, token->factory.user_data);
	return 0;
    }
...
extern "C" int
bobol_host_factory_instance_bind_controller(
...
extern "C" void
bobol_host_factory_instance_destroy(bobol_host_factory_token_t *token,
	void *instance)
{
    if (!token || !instance)
	return;
    ...
    (void)bobol_host_factory_instance_bind_controller(token, instance,
	NULL);
    token->factory.destroy(instance, token->factory.user_data);
}

extern "C" int
bobol_host_factory_instance_request_frame(
	bobol_host_factory_token_t *token, void *instance, const char *reason)
{
...
extern "C" int
bobol_host_factory_instance_resize(bobol_host_factory_token_t *token,
...
extern "C" int
bobol_host_factory_instance_capture(bobol_host_factory_token_t *token,
...
extern "C" int
bobol_host_factory_instance_dimensions(bobol_host_factory_token_t *token,
	void *instance, unsigned int *width, unsigned int *height,
	double *device_pixel_ratio)
{
...
extern "C" int
bobol_host_factory_instance_set_title(bobol_host_factory_token_t *token,
...
extern "C" int
bobol_host_factory_instance_set_visible(bobol_host_factory_token_t *token,
...
extern "C" int
bobol_host_factory_instance_set_vsync(bobol_host_factory_token_t *token,
...
extern "C" void *
bobol_host_factory_instance_framebuffer_window_host(
