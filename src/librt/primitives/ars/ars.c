...
C_DECL int
rt_ars_plot(struct bu_list *vhead, struct rt_db_internal *ip, const struct bg_tess_tol *UNUSED(ttol), const struct bn_tol *UNUSED(tol), const struct bv_view_info *UNUSED(info))
{
    register size_t i;
    register size_t j;
    struct rt_ars_internal *arip;
	...
	v1 = arip->curves[i];
	RT_ADD_VLIST(vlfree, vhead, v1, RT_VLIST_LINE_MOVE);
	v1 += ELEMENTS_PER_VECT;
	for (j = 1; j <= arip->pts_per_curve; j++, v1 += ELEMENTS_PER_VECT)
	    RT_ADD_VLIST(vlfree, vhead, v1, RT_VLIST_LINE_DRAW);
    }

    /* Connect the Ith points on each curve, to make a mesh.  */
    for (i = 0; i < arip->pts_per_curve; i++) {
	RT_ADD_VLIST(vlfree, vhead, &arip->curves[0][i*ELEMENTS_PER_VECT], RT_VLIST_LINE_MOVE);
	for (j = 1; j < arip->ncurves; j++)
	    RT_ADD_VLIST(vlfree, vhead, &arip->curves[j][i*ELEMENTS_PER_VECT], RT_VLIST_LINE_DRAW);
    }
