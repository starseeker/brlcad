...
nmg_make_dualvu(struct vertex *v, struct faceuse *fu, const struct bn_tol *tol)
{
	...
	for (BU_LIST_FOR(eu, edgeuse, &lu->down_hd)) {
	    ...
	    code = bg_dist_pnt3_lseg3(&dist, pca,
				     eu->vu_p->v_p->vg_p->coord,
				     eu->eumate_p->vu_p->v_p->vg_p->coord,
				     v->vg_p->coord, tol);

	    if (nmg_debug & NMG_DEBUG_POLYSECT)
		bu_log("bg_dist_pnt3_lseg3 returns %d, dist=%f\n", code, dist);
...
/* distance along intersect ray for this vu */
{
    ...
    if ((nmg_debug & NMG_DEBUG_VERIFY) && is->fu1 && is->fu2) {
...
/* ptr to faceuse/edgeuse associated w/2d projection */
{
	...
	dist = DIST_PNT_PLANE(vg->coord, n);
...
nmg_isect_vert2p_face2p(struct nmg_inter_struct *is, struct vertexuse *vu1, struct faceuse *fu2)
{
	...
	if (BU_LIST_FIRST_MAGIC(&lu2->down_hd) == NMG_VERTEXUSE_MAGIC) {
	    ...
	    if (bg_pnt3_pnt3_equal(pt, vu2->v_p->vg_p->coord, &is->tol)) {
...
nmg_isect_3vertex_3face(struct nmg_inter_struct *is, struct vertexuse *vu, struct faceuse *fu)
{
    ...
    if (vup) {
	if (nmg_debug & NMG_DEBUG_POLYSECT) bu_log("\tvu lies in face (topology 1)\n");
    ...
    NMG_GET_FU_PLANE(n, fu);
    dist = DIST_PNT_PLANE(pt, n);

    if (!NEAR_ZERO(dist, is->tol.dist)) {
	if (nmg_debug & NMG_DEBUG_POLYSECT) bu_log("\tvu not on face (geometry)\n");
	return;
    }
...
/* Edge to be broken (in fu1) */
{
    ...
    if (bg_pnt3_pnt3_equal(hit_pt, v1->vg_p->coord, &is->tol))
	bu_bomb("nmg_break_3edge_at_plane() hit_pt equal to v1\n");
    if (bg_pnt3_pnt3_equal(hit_pt, v1mate->vg_p->coord, &is->tol))
	...
	dist = DIST_PNT_PNT(hit_pt, v1->vg_p->coord);
	if (bg_pnt3_pnt3_equal(hit_pt, v1->vg_p->coord, &t2))
	    bu_log("NOTICE: nmg_break_3edge_at_plane() hit_pt nearly equal to v1 %g*tol\n", dist/is->tol.dist);
	dist = DIST_PNT_PNT(hit_pt, v1mate->vg_p->coord);
	if (bg_pnt3_pnt3_equal(hit_pt, v1mate->vg_p->coord, &t2))
	 ...
	 */
	int nmg_class;
	nmg_class = nmg_class_pnt_fu_except(hit_pt, fu2,
					   NULL,
					   NULL,
					   NULL,
					   NULL, 0,
	...
	switch (nmg_class) {
		...
		break;
	    default:
		bu_bomb("nmg_break_3edge_at_plane() bad classification return from nmg_class_pnt_f()\n");
	}
...

{
    ...
    dist = -INFINITY;
    code = bg_isect_pnt2_lseg2(&dist, a, b, p, &(is->tol));
...
nmg_break_eg_on_v(const struct edge_g_lseg *eg, struct vertex *v, const struct bn_tol *tol)
{
    ...
    VMOVE(dir, eg->e_dir);
    VUNITIZE(dir);
    vdist = bg_dist_pnt3_along_line3(eg->e_pt, dir, v->vg_p->coord);
	...
	if (bg_pnt3_pnt3_equal(v->vg_p->coord, va->vg_p->coord, tol)) {
	    nmg_jv(v, va);
	    continue;
	}
	if (bg_pnt3_pnt3_equal(v->vg_p->coord, vb->vg_p->coord, tol)) {
	    nmg_jv(v, vb);
	    continue;
	}
	a = bg_dist_pnt3_along_line3(eg->e_pt, dir, va->vg_p->coord);
	b = bg_dist_pnt3_along_line3(eg->e_pt, dir, vb->vg_p->coord);
	if (NEAR_EQUAL(a, vdist, tol->dist)) continue;
	if (NEAR_EQUAL(b, vdist, tol->dist)) continue;
	if (!bg_between(a, vdist, b, tol)) continue;
...
/* fu of eu2, for error checks */
{
    ...
    status = bg_isect_line2_lseg2(dist, eu1_start, eu1_dir,
				  eu2_start, eu2_dir, &is->tol);

    if (nmg_debug & NMG_DEBUG_POLYSECT) {
	bu_log("\tbg_isect_line2_lseg2()=%d, dist: %g, %g\n",
	...
	if (ZERO(dist[1])) {
	    ...
	    new_v2 = nmg_find_pnt_in_model(m, hit_pt, &(is->tol));
...
nmg_isect_wireedge3p_face3p(struct nmg_inter_struct *is, struct edgeuse *eu1, struct faceuse *fu2, struct bu_list *vlfree)
{
    ...
    status = bg_isect_line3_plane(&dist, start_pt, edge_vect,
				  n2, &is->tol);

    if (nmg_debug & NMG_DEBUG_POLYSECT) {
	if (status >= 0)
	    bu_log("\tHit. bg_isect_line3_plane=%d, dist=%g (%e)\n",
		   status, dist, dist);
	else
	    bu_log("\tMiss. Boring status of bg_isect_line3_plane: %d\n",
		   status);
    }
    if (status == 0) {
	...
	ret = nmg_isect_edge2p_face2p(&is2, eu1, fu2, fu1, vlfree);
	...
	if (nmg_debug & NMG_DEBUG_POLYSECT && ret == 0) {
    ...
    {
	fastf_t ff_dist;
	ff_dist = bg_dist_line3_pnt3(is->pt, is->dir, hit_pt);
	 ...
	 * Insert new vertex at intersection point.
	 */
	vu2_final = nmg_break_3edge_at_plane(hit_pt, fu2, is, eu1, vlfree);
	if (vu2_final)
...
nmg_isect_wireloop3p_face3p(struct nmg_inter_struct *bs, struct loopuse *lu, struct faceuse *fu, struct bu_list *vlfree)
{
	...
	discards += nmg_isect_wireedge3p_face3p(bs, eu, fu, vlfree);
...
/* fu that eu1 is from */
{
    ...
    if (fu1 && nmg_debug & (NMG_DEBUG_POLYSECT|NMG_DEBUG_FCUT|NMG_DEBUG_MESH)
	&& nmg_debug & NMG_DEBUG_PLOTEM) {
	nmg_pl_2fu("Iface%d.plot3", fu2, fu1, 0, vlfree);
    }
	...
	nmg_region_v_unique(is->s1->r_p, vlfree, &is->tol);
	nmg_region_v_unique(is->s2->r_p, vlfree, &is->tol);
    }

    /* Build list of all edgeuses in eu1/fu1 and fu2 */
    if (fu1) {
	nmg_edgeuse_tabulate(&eu1_list, &fu1->l.magic, vlfree);
    } else {
	nmg_edgeuse_tabulate(&eu1_list, &eu1->l.magic, vlfree);
    }
    ...
    nmg_isect_line2_face2pNEW(is, fu2, fu1, &eu2_list, &eu1_list, vlfree);

    /* If eu1 is a wire, there is no fu1 to run line through. */
    if (fu1) {
	/* We are intersecting with ourselves */
	nmg_isect_line2_face2pNEW(is, fu1, fu2, &eu1_list, &eu2_list, vlfree);
    }
    ...
    if (fu1 && fu2) {
	is->on_eg = nmg_face_cutjoin(&vert_list1, &vert_list2, mag1, mag2, fu1, fu2, is->pt, is->dir, is->on_eg, vlfree, &is->tol);
...
nmg_coplanar_face_vertex_fuse(struct faceuse *fu1, struct faceuse *fu2, struct bu_list *vlfree, struct bn_tol *tol)
{
    ...
    (void)nmg_vertex_fuse((const uint32_t *)&verts, vlfree, tol);
...
nmg_isect_two_face2p_jra(struct nmg_inter_struct *is, struct faceuse *fu1, struct faceuse *fu2, struct bu_list *vlfree)
{
    ...
    nmg_coplanar_face_vertex_fuse(fu1, fu2, vlfree, &is->tol);
    ...
    for (i = 0; i < BU_PTBL_LEN(&eu1_list); i++) {
	struct edgeuse *eu1;
	...
	for (j = 0; j < BU_PTBL_LEN(&eu2_list); j++) {
	    ...
	    code = bg_isect_lseg3_lseg3(dist, vg1a->coord, vt1_3d,
		 ...
		 * returned from function 'bg_isect_lseg3_lseg3' is the
		...
		if (ZERO(dist[hit_no])) {
		    ...
		    if ((hit_vu = nmg_find_pnt_in_face(fu2, hit_pt, &is->tol))) {
			hitv = hit_vu->v_p;
		    } else if ((hit_vu = nmg_find_pnt_in_face(fu1, hit_pt, &is->tol))) {
			hitv = hit_vu->v_p;
		    } else {
			hitv = nmg_find_pnt_in_model(nmg_find_model(&fu1->l.magic), hit_pt, &is->tol);
		    }
		...
		if (UNLIKELY(nmg_debug & NMG_DEBUG_POLYSECT && hitv)) {
		...
		if (hitv) {
		    (void)nmg_break_all_es_on_v(&fu1->l.magic, hitv, vlfree, &is->tol);
		    (void)nmg_break_all_es_on_v(&fu2->l.magic, hitv, vlfree, &is->tol);
		}
	    }
	}
    }
     ...
     * (if they overlap)
     */
    nmg_vertex_tabulate(&v_list, &fu1->l.magic, vlfree);

    for (i=0; i<BU_PTBL_LEN(&v_list); i++) {
	struct vertex *v;
	int nmg_class;
	...
	nmg_class = nmg_class_pnt_fu_except(v->vg_p->coord, fu2, NULL, NULL, NULL,
					   (char *)NULL, 0, 0, vlfree, &is->tol);
    ...
    /* same for fu2 */
    nmg_vertex_tabulate(&v_list, &fu2->l.magic, vlfree);
	...
	/* Check if this vertex is within other FU */
	nmg_class = nmg_class_pnt_fu_except(v->vg_p->coord, fu1, NULL, NULL, NULL,
					   (char *)NULL, 0, 0, vlfree, &is->tol);
	...
	for (BU_LIST_FOR(eu, edgeuse, &lu->down_hd)) {
	    ...
	    nmg_fcut_face_2d(is->l2, is->mag2, fu2, fu1, vlfree, &is->tol);
	...
	for (BU_LIST_FOR(eu, edgeuse, &lu->down_hd)) {
	    ...
	    nmg_fcut_face_2d(is->l1, is->mag1, fu1, fu2, vlfree, &is->tol);
...
nmg_isect_line2_edge2p(struct nmg_inter_struct *is, struct bu_ptbl *list, struct edgeuse *eu1, struct faceuse *fu1, struct faceuse *fu2)
{
    ...
    status = bg_isect_line2_lseg2(dist, is->pt2d, is->dir2d,
	...
	if (!bg_pnt3_pnt3_equal(hit_pt, vu1a->v_p->vg_p->coord, &(is->tol)))
	...
	if (!bg_pnt3_pnt3_equal(hit_pt, vu1b->v_p->vg_p->coord, &(is->tol)))
	...
	if (nmg_debug & NMG_DEBUG_POLYSECT) {
	    ...
	    code = bg_isect_pnt_lseg(&distance, vu1a->v_p->vg_p->coord,
				    vu1b->v_p->vg_p->coord,
				    hit_pt, &(is->tol));
	    bu_log("\tbg_isect_pnt_lseg() dist=%g, ret=%d\n", distance, code);
	    if (code < 0) bu_bomb("3D point not on 3D lseg\n");

	    /* Ensure that the 3D hit_pt is between the end pts */
	    if (!bg_between(vu1a->v_p->vg_p->coord[X], hit_pt[X], vu1b->v_p->vg_p->coord[X], &(is->tol)) ||
		!bg_between(vu1a->v_p->vg_p->coord[Y], hit_pt[Y], vu1b->v_p->vg_p->coord[Y], &(is->tol)) ||
		!bg_between(vu1a->v_p->vg_p->coord[Z], hit_pt[Z], vu1b->v_p->vg_p->coord[Z], &(is->tol))) {
		VPRINT("vu1a", vu1a->v_p->vg_p->coord);
		VPRINT("hitp", hit_pt);
		VPRINT("vu1b", vu1b->v_p->vg_p->coord);
...
nmg_isect_line2_vertex2(struct nmg_inter_struct *is, struct vertexuse *vu1, struct faceuse *fu1)
{
    ...
    if (bg_distsq_line3_pnt3(is->pt, is->dir, vu1->v_p->vg_p->coord) > is->tol.dist_sq)
	return;
...
nmg_find_eg_on_line(const uint32_t *magic_p, const point_t pt, const vect_t dir, struct bu_list *vlfree, const struct bn_tol *tol)
{
    ...
    nmg_edgeuse_on_line_tabulate(&eutab, magic_p, pt, dir, vlfree, tol);
...

{
    ...
    bu_log("dist v-hit=%g, equal=%d\n",
	   bg_dist_pnt3_pnt3(v->vg_p->coord, hit_v->vg_p->coord),
	   bg_pnt3_pnt3_equal(v->vg_p->coord, hit_v->vg_p->coord, tol)
	);
    if (eg1) {
	if (bg_2line3_colinear(eg1->e_pt, eg1->e_dir, eg2->e_pt, eg2->e_dir, 1e5, tol))
	    bu_bomb("ERROR: nmg_repair_v_near_v() eg1 and eg2 are collinear!\n");
	bu_log("eg1: line/ vu dist=%g, hit dist=%g\n",
	       bg_dist_line3_pnt3(eg1->e_pt, eg1->e_dir, v->vg_p->coord),
	       bg_dist_line3_pnt3(eg1->e_pt, eg1->e_dir, hit_v->vg_p->coord));
	bu_log("eg2: line/ vu dist=%g, hit dist=%g\n",
	       bg_dist_line3_pnt3(eg2->e_pt, eg2->e_dir, v->vg_p->coord),
	       bg_dist_line3_pnt3(eg2->e_pt, eg2->e_dir, hit_v->vg_p->coord));
	nmg_pr_eg(&eg1->l.magic, 0);
	nmg_pr_eg(&eg2->l.magic, 0);
    }

    if (bg_dist_pnt3_pnt3(v->vg_p->coord,
...

{
    ...
    if (bg_distsq_line3_pnt3(eg1->e_pt, eg1->e_dir, v->vg_p->coord) > tol->dist_sq) {
	VPRINT("v", v->vg_p->coord);
...
int
nmg_is_vertex_on_inter(struct vertex *v, struct faceuse *fu1, struct faceuse *fu2, struct nmg_inter_struct *is, struct bu_list *vlfree)
{
    struct vertex_g *vg;
    plane_t pl1, pl2;
    int code;
    ...
    dist = fabs(DIST_PNT_PLANE(vg->coord, pl1));
    if (dist > is->tol.dist)
	return 0;
    dist = fabs(DIST_PNT_PLANE(vg->coord, pl2));
    if (dist > is->tol.dist)
	return 0;

    /* check if it is on intersection line */
    if (bg_distsq_line3_pnt3(is->pt, is->dir, vg->coord) > is->tol.dist_sq)
	return 0;

    /* check if it is within fu's */
    code = nmg_class_pnt_fu_except(vg->coord, fu1, (struct loopuse *)NULL,
				  NULL,
				  NULL,
    ...
    code = nmg_class_pnt_fu_except(vg->coord, fu2, NULL,
				  NULL,
				  NULL,
				  NULL, 0, 0, vlfree, &is->tol);
    if (code != NMG_CLASS_AinB)
...
nmg_isect_eu_verts(struct edgeuse *eu, struct vertex_g *vg1, struct vertex_g *vg2, struct bu_ptbl *verts, struct bu_ptbl *inters, const struct bn_tol *tol)
{
	...
	code = bg_dist_pnt3_lseg3(&dist, pca, vg1->coord,
...
nmg_isect_eu_eu(struct edgeuse *eu1, struct vertex_g *vg1a, struct vertex_g *vg1b, vect_t dir1, struct edgeuse *eu2, struct bu_ptbl *verts, struct bu_ptbl *inters, const struct bn_tol *tol)
{
    ...
    code = bg_isect_lseg3_lseg3(dist, vg1a->coord, dir1, vg2a->coord, dir2, tol);
	...
	v = nmg_find_pnt_in_model(m, hit_pt, tol);
...
void
nmg_isect_eu_fu(struct nmg_inter_struct *is, struct bu_ptbl *verts, struct edgeuse *eu, struct faceuse *fu, struct bu_list *vlfree)
{
    struct model *m;
	...
	bu_log("nmg_isect_eu_fu() called with EU (%p) from model (%p)\n",
	       (void *)eu, (void *)nmg_find_model(&eu->l.magic));
	bu_log("\tand FU (%p) from model (%p)\n", (void *)fu, (void *)m);
    ...
    if (bg_isect_line3_plane(&dist, vg1->coord, dir, pl, &is->tol) < 1) {
	if (UNLIKELY(nmg_debug & NMG_DEBUG_POLYSECT))
	 ...
	 */
	dist_to_plane = fabs(DIST_PNT_PLANE(vg1->coord, pl));
	if (dist_to_plane < is->tol.dist) {
	    /* check if hit point is within fu */
	    nmg_class = nmg_class_pnt_fu_except(vg1->coord, fu, (struct loopuse *)NULL,
					       0, 0, (char *)NULL, 0, 0, vlfree, &is->tol);
	    if (nmg_class != NMG_CLASS_AinB)
		goto out;
	...
	dist_to_plane = fabs(DIST_PNT_PLANE(vg2->coord, pl));
	if (dist_to_plane < is->tol.dist) {
	    /* check if hit point is within fu */
	    nmg_class = nmg_class_pnt_fu_except(vg2->coord, fu, (struct loopuse *)NULL,
					       0, 0, (char *)NULL, 0, 0, vlfree, &is->tol);
	    if (nmg_class != NMG_CLASS_AinB)
		goto out;
	...
	if (bg_pnt3_pnt3_equal(hit_pt, vg1->coord, &is->tol)) {
	    v = eu->vu_p->v_p;
	    VMOVE(hit_pt, vg1->coord);
	} else if (bg_pnt3_pnt3_equal(hit_pt, vg2->coord, &is->tol)) {
	    v = eu->eumate_p->vu_p->v_p;
	...
	/* check if hit point is within fu */
	nmg_class = nmg_class_pnt_fu_except(hit_pt, fu, (struct loopuse *)NULL,
					   0, 0, (char *)NULL, 0, 0, vlfree, &is->tol);
	...
	if (nmg_class == NMG_CLASS_AinB) {
	    ...
	    if (!v)
		v = nmg_find_pnt_in_model(m, hit_pt, &is->tol);
...
void
nmg_isect_fu_jra(struct nmg_inter_struct *is, struct faceuse *fu1, struct faceuse *fu2, struct bu_ptbl *eu1_list, struct bu_ptbl *eu2_list, struct bu_list *vlfree)
{
    struct model *m;
    ...
    nmg_vertex_tabulate(&verts2, &fu2->l.magic, vlfree);
    nmg_edgeuse_tabulate(&eus, &fu1->l.magic, vlfree);
	...
	nmg_isect_eu_fu(is, &verts2, eu, fu2, vlfree);
    }
    bu_ptbl_free(&verts2);
    bu_ptbl_free(&eus);

    /* Intersect fu2 edgeuses */
    nmg_vertex_tabulate(&verts1, &fu1->l.magic, vlfree);
    nmg_edgeuse_tabulate(&eus, &fu2->l.magic, vlfree);
    for (i = 0; i < BU_PTBL_LEN(&eus); i++) {
	struct edgeuse *eu;
	...
	nmg_isect_eu_fu(is, &verts1, eu, fu1, vlfree);
    }
    bu_ptbl_free(&verts1);
    bu_ptbl_free(&eus);
    ...
    /* merge the two lists */
    for (i=0; i<BU_PTBL_LEN(&verts2); i++) {
	struct vertex *v;
    ...
    for (i=0; i< BU_PTBL_LEN(&verts1); i++) {
	struct vertex *v;
	struct vertexuse *vu;
	fastf_t dist;
	...
	if (!nmg_is_vertex_on_inter(v, fu1, fu2, is, vlfree))
	    continue;
...
re_tabulate:
    ...
    nmg_edge_g_tabulate(&eg_list, &fu1->l.magic, vlfree);
	...
	colinear:
	    ...
	    for (eu1_index=0; eu1_index < BU_PTBL_LEN(eu1_list); eu1_index++) {
		...
		NMG_CK_VERTEX_G(vg);
		(void)bg_dist_pnt3_line3(&distance, pca, is->pt, is->dir, vg->coord, &(is->tol));
		if (distance <= is->tol.dist) {
		    /* vertex is on intersection line */

		    if (DIST_PNT_PLANE(vg->coord, pl2) <= is->tol.dist) {
			/* and in plane of fu2 */
			if (nmg_class_pnt_fu_except(vg->coord, fu2, (struct loopuse *)NULL, NULL, NULL, (char *)NULL, 0, 0, vlfree, &(is->tol)) != NMG_CLASS_AoutB) {
			    /* and within fu2 */
		...
		NMG_CK_VERTEX_G(vg);
		bg_dist_pnt3_line3(&distance, pca, is->pt, is->dir, eu_end->vu_p->v_p->vg_p->coord, &(is->tol));
		if (distance <= is->tol.dist) {
		    /* vertex is on intersection line */
	    ...
	    for (eu2_index=0; eu2_index < BU_PTBL_LEN(eu2_list); eu2_index++) {
		...
		NMG_CK_VERTEX_G(vg);
		bg_dist_pnt3_line3(&distance, pca, is->pt, is->dir, vg->coord, &(is->tol));
		if (distance <= is->tol.dist) {
		    /* vertex is on intersection line */

		    if (DIST_PNT_PLANE(vg->coord, pl1) <= is->tol.dist) {
			/* and in plane of fu1 */
			if (nmg_class_pnt_fu_except(vg->coord, fu1, (struct loopuse *)NULL, NULL, NULL, (char *)NULL, 0, 0, vlfree, &(is->tol)) != NMG_CLASS_AoutB) {
			    /* and within fu1 */
			    distance = VDIST(is->pt, vg->coord);
	...
	code = bg_isect_line2_line2(dist, is->pt2d, is->dir2d,
	...
	/* Double check */
	if (is->on_eg && bg_2line3_colinear(
		(*eg1)->e_pt, (*eg1)->e_dir,
	...
	if (is->on_eg && !hit_v) {
		 ...
		 * face cutter.
		 */
		if (!V3PNT_IN_RPP(hit_v->vg_p->coord, fu1->f_p->min_pt, fu1->f_p->max_pt) &&
		    !V3PNT_IN_RPP(hit_v->vg_p->coord, fu2->f_p->min_pt, fu2->f_p->max_pt)
		    ) {
		...
		NMG_GET_FU_PLANE(n2, fu2);
		dist1 = DIST_PNT_PLANE(hit_v->vg_p->coord, n1);
		dist2 = DIST_PNT_PLANE(hit_v->vg_p->coord, n2);

		if (!NEAR_ZERO(dist1, is->tol.dist) || !NEAR_ZERO(dist2, is->tol.dist)) {
		    continue;
		}
		nmg_class=nmg_class_pnt_fu_except(hit_v->vg_p->coord, fu1, (struct loopuse *)NULL, NULL, NULL, (char *)NULL, 0, 0, vlfree, &(is->tol));
		if (nmg_class == NMG_CLASS_AoutB) {
		    if (nmg_debug & NMG_DEBUG_POLYSECT) {
			VPRINT("\t\tisect pt outside face fu1 (nmg_class_pnt_fu_except):", hit_v->vg_p->coord);
		    }
		    continue;
		} else if (nmg_class_pnt_fu_except(hit_v->vg_p->coord, fu2, (struct loopuse *)NULL, NULL, NULL, (char *)NULL, 0, 0, vlfree, &(is->tol)) == NMG_CLASS_AoutB) {
		    if (nmg_debug & NMG_DEBUG_POLYSECT) {
			VPRINT("\t\tisect pt outside face fu2 (nmg_class_pnt_fu_except):", hit_v->vg_p->coord);
		    }
		    continue;
		}
	    ...
	    bu_log(" 3d line isect, code=%d\n",
		   bg_isect_line3_line3(&dist[0], &dist[1],
					is->pt, is->dir,
					(*eg1)->e_pt,
					(*eg1)->e_dir,
					&(is->tol)));
	...
	if (!V3PNT_IN_RPP(hit3d, fu1->f_p->min_pt, fu1->f_p->max_pt)) {
	...
	nmg_class = nmg_class_pnt_fu_except(hit3d, fu1, (struct loopuse *)NULL, NULL, NULL, (char *)NULL, 0, 0, vlfree, &(is->tol));
	if (nmg_class == NMG_CLASS_AoutB) {
	    if (nmg_debug & NMG_DEBUG_POLYSECT) {
		VPRINT("\t\tisect pt outside face fu1 (nmg_class_pnt_fu_except):", hit3d);
	    }
	    continue;
	} else if ((nmg_class=nmg_class_pnt_fu_except(hit3d, fu2, (struct loopuse *)NULL, NULL, NULL, (char *)NULL, 0, 0, vlfree, &(is->tol))) == NMG_CLASS_AoutB) {
	    if (nmg_debug & NMG_DEBUG_POLYSECT) {
		VPRINT("\t\tisect pt outside face fu2 (nmg_class_pnt_fu_except):", hit3d);
	    }
	    continue;
	} else if (nmg_debug & NMG_DEBUG_POLYSECT) {
	    bu_log("\t\tnmg_class_pnt_fu_except(fu1) returns %s\n", nmg_class_name(nmg_class));
	}
	...
	for (eu1_index=0; eu1_index < BU_PTBL_LEN(eu1_list); eu1_index++) {
	     ...
	     */
	    tmp_dist_sq = bg_distsq_line3_pnt3(is->pt, is->dir, vu1a->v_p->vg_p->coord);
	    if (nmg_debug & NMG_DEBUG_POLYSECT) {
		...
		/* Fall through to bg_isect_pnt2_lseg2() */
	    }
	    tmp_dist_sq = bg_distsq_line3_pnt3(is->pt, is->dir, vu1b->v_p->vg_p->coord);
	    if (nmg_debug & NMG_DEBUG_POLYSECT) {
	    ...
	    ldist = 0;
	    code = bg_isect_pnt2_lseg2(&ldist, eu1_pt2d, eu1_end2d, hit2d, &(is->tol));
	    if (nmg_debug & NMG_DEBUG_POLYSECT) {
		bu_log("\tbg_isect_pnt2_lseg2() returned %d, ldist=%g\n", code, ldist);
	    }
	    switch (code) {
		case -2:
		...
		case 3:
		     ...
		     */
		    if (!hit_v) {
			hit_v = nmg_find_pnt_in_model(fu1->s_p->r_p->m_p,
						     hit3d, &(is->tol));
	...
	/* Needs to be a 3D comparison */
	if (bg_distsq_line3_pnt3(is->pt, is->dir,
...
nmg_is_eu_on_line3(const struct edgeuse *eu, const point_t UNUSED(pt), const vect_t dir, const struct bn_tol *tol)
{
    ...
    if (bg_distsq_line3_pnt3(eg->e_pt, eg->e_dir,
 ...
 */
struct edge_g_lseg *
nmg_find_eg_between_2fg(const struct faceuse *ofu1, const struct faceuse *fu2, struct bu_list *vlfree, const struct bn_tol *tol)
{
	...
	restart:
		...
		for (
			...
			if (nmg_debug & NMG_DEBUG_BASIC) {
			    nmg_pl_lu_around_eu(eur, vlfree);
			}
			ret = eur;
			continue;
		    }
...
nmg_isect_two_face3p(struct nmg_inter_struct *is, struct faceuse *fu1, struct faceuse *fu2, struct bu_list *vlfree)
{
    ...
    if (nmg_debug & (NMG_DEBUG_POLYSECT|NMG_DEBUG_FCUT|NMG_DEBUG_MESH)
	&& nmg_debug & NMG_DEBUG_PLOTEM) {
	nmg_pl_2fu("Iface%d.plot3", fu1, fu2, 0, vlfree);
    }
...
/* nmg_isect_line2_face2pNEW(is, fu2, fu1, &eu2_list, &eu1_list); */
    ...
    nmg_isect_fu_jra(is, fu1, fu2, &eu1_list, &eu2_list, vlfree);
	...
	nmg_fu_touchingloops(fu1);
	nmg_fu_touchingloops(fu2);
	nmg_region_v_unique(fu1->s_p->r_p, vlfree, &is->tol);
	nmg_region_v_unique(fu2->s_p->r_p, vlfree, &is->tol);
	nmg_vfu(&fu1->s_p->fu_hd, fu1->s_p);
	nmg_vfu(&fu2->s_p->fu_hd, fu2->s_p);
    }

    nmg_mesh_faces(fu1, fu2, vlfree, &is->tol);
    if (nmg_debug & NMG_DEBUG_VERIFY) {
	nmg_fu_touchingloops(fu1);
	nmg_fu_touchingloops(fu2);
    }
...
nmg_cut_lu_into_coplanar_and_non(struct loopuse *lu, plane_t pl, struct nmg_inter_struct *is, struct bu_list *vlfree)
{
    ...
    NMG_CK_VERTEX_G(vg2);
    dist2 = DIST_PNT_PLANE(vg2->coord, pl);
	...
	for (i = 0; i < BU_PTBL_LEN(&cut_list); i++) {
	    ...
	    for (j=i; j<BU_PTBL_LEN(&cut_list); j++) {
		fastf_t tmp_dist;
	...
	while (!done) {
	    ...
	    for (i=1; i<BU_PTBL_LEN(&cut_list); i++) {
	...
	for (BU_LIST_FOR(lu1, loopuse, &is->fu1->lu_hd)) {
		...
		code = bg_isect_lseg3_lseg3(dists, is->pt, dir,
		...
		if (code == 0) {
			...
			for (BU_LIST_FOR(vu, vertexuse, &eu1->eumate_p->vu_p->v_p->vu_hd)) {
			    if (nmg_find_fu_of_vu(vu) == is->fu1)
				// Note - this is using len rather than dist for the last
				// parameter, and the dist calculation is unused??  Use of
				// len dates back to SVN commit id 11890 by John Anderson
				// in 1996, but the dist calculation was also added at
				// that time.
				nmg_enlist_one_vu(is, vu, len);
			}
		    }
		...
		hit_vu = nmg_find_pnt_in_face(is->fu1, hit_pt, &is->tol);
		if (hit_vu)
		    hit_v = hit_vu->v_p;

		if (!hit_v)
		    hit_v = nmg_find_pnt_in_model(m, hit_pt, &is->tol);
	...
	(void)nmg_face_cutjoin(is->l1, is->l2, is->mag1, is->mag2, is->fu1,
			       is->fu2, is->pt, is->dir, is->on_eg, vlfree, &is->tol);
...
void
nmg_check_radial_angles(char *str, struct shell *s, struct bu_list *vlfree, const struct bn_tol *tol)
{
 ...
 */
int
nmg_faces_can_be_intersected(struct nmg_inter_struct *bs, const struct faceuse *fu1, const struct faceuse *fu2, struct bu_list *vlfree, const struct bn_tol *tol)
{
    plane_t pl1, pl2;
    point_t min_pt;
    struct face *f1, *f2;
    ...
    if (bg_make_pnt_3planes(bs->pt, tmp_pl, pl1, pl2))
	return 0;
	...
	dist = DIST_PNT_PLANE(v->vg_p->coord, pl2);
	...
	VSUB2(to_v, v->vg_p->coord, pca);
	dist = DIST_PNT_PLANE(v->vg_p->coord, pl1);
	if (VDOT(to_v, left) > 0.0) {
	    /* left of intersection line */
	    if (dist > tol->dist)
		above_left++;
...
nmg_no_isect_fu_pl(struct faceuse *fu1, struct faceuse *fu2, const struct bn_tol *tol)
{
	...
	for (BU_LIST_FOR (eu, edgeuse, &lu->down_hd)) {
	    ...
	    dist = DIST_PNT_PLANE(eu->vu_p->v_p->vg_p->coord, pl);
 ...
 */
void
nmg_isect_two_generic_faces(struct faceuse *fu1, struct faceuse *fu2, struct bu_list *vlfree, const struct bn_tol *tol)
{
	...
	case 0:
	    bs.coplanar = 0;
	    nmg_isect_two_face3p(&bs, fu1, fu2, vlfree);
	    break;
	case -1:
	    /* co-planar faces */
	    bs.coplanar = 1;
	    nmg_isect_two_face2p_jra(&bs, fu1, fu2, vlfree);
	    break;
	case -2:
    ...
    (void)nmg_unbreak_region_edges(&fu1->l.magic, vlfree);
    (void)nmg_unbreak_region_edges(&fu2->l.magic, vlfree);
	...
	nmg_face_plot(fu1, vlfree);
	nmg_face_plot(fu2, vlfree);

	sprintf(name, "shellA%d.plot3", nshell);
	if ((fp = fopen(name, "wb")) != NULL) {
	    bu_log("overlay %s\n", name);
	    nmg_pl_s(fp, fu1->s_p, vlfree);
	    fclose(fp);
	}
	...
	if ((fp = fopen(name, "wb")) != NULL) {
	    bu_log("overlay %s\n", name);
	    nmg_pl_s(fp, fu2->s_p, vlfree);
	    fclose(fp);
	}
    }

    if (nmg_debug & NMG_DEBUG_VERIFY) {
	nmg_region_v_unique(fu1->s_p->r_p, vlfree, &bs.tol);
	nmg_region_v_unique(fu2->s_p->r_p, vlfree, &bs.tol);
	nmg_fu_touchingloops(fu1);
	nmg_fu_touchingloops(fu2);
	nmg_ck_face_worthless_edges(fu1);
...
nmg_isect_edge3p_edge3p(struct nmg_inter_struct *is, struct edgeuse *eu1, struct edgeuse *eu2)
{
    ...
    status = bg_isect_lseg3_lseg3(dist,
...
nmg_isect_vertex3_edge3p(struct nmg_inter_struct *is, struct vertexuse *vu1, struct edgeuse *eu2)
{
    ...
    code = bg_isect_pnt_lseg(&dist, eu2->vu_p->v_p->vg_p->coord,
...
nmg_isect_edge3p_shell(struct nmg_inter_struct *is, struct edgeuse *eu1, struct shell *s2, struct bu_list *vlfree)
{
	...
	(void)nmg_isect_wireedge3p_face3p(is, eu1, fu2, vlfree);
    }
    ...
    if (nmg_class_pnt_s(midpt, s2, 0, vlfree, &is->tol) == NMG_CLASS_AoutB)
 ...
 */
void
nmg_crackshells(struct shell *s1, struct shell *s2, struct bu_list *vlfree, const struct bn_tol *tol)
{
    ...
    if (UNLIKELY(nmg_debug & NMG_DEBUG_VERIFY)) {
	nmg_ck_vs_in_region(s1->r_p, vlfree, tol);
	nmg_ck_vs_in_region(s2->r_p, vlfree, tol);
    }
    ...
    nmg_face_tabulate(&faces1, &s1->l.magic, vlfree);
    nmg_face_tabulate(&faces2, &s2->l.magic, vlfree);
    ...
    for (i = 0; i < (size_t)BU_PTBL_LEN(&faces1); i++) {
	...
	for (j = 0; j < (size_t)BU_PTBL_LEN(&faces2); j++) {
	    ...
	    nmg_isect_two_generic_faces(fu1, fu2, vlfree, tol);
	}
	...
	for (BU_LIST_FOR(lu2, loopuse, &s2->lu_hd)) {
	    ...
	    (void)nmg_isect_wireloop3p_face3p(&is, lu2, fu1, vlfree);
	}
	/* Check f1 from s1 against wire edges of s2 */
	for (BU_LIST_FOR(eu2, edgeuse, &s2->eu_hd)) {
	    NMG_CK_EDGEUSE(eu2);
	    nmg_isect_wireedge3p_face3p(&is, eu2, fu1, vlfree);
	}
	...
	for (BU_LIST_FOR(eu1, edgeuse, &lu1->down_hd)) {
	    ...
	    nmg_isect_edge3p_shell(&is, eu1, s2, vlfree);
	}
    }
    ...
    /* Eliminate stray vertices that were added along edges in this step */
    (void)nmg_unbreak_region_edges(&s1->l.magic, vlfree);
    (void)nmg_unbreak_region_edges(&s2->l.magic, vlfree);
		   ...
		   const struct bn_tol *tol)
{
	...
	for (BU_LIST_FOR(eu, edgeuse, &lu->down_hd)) {
	    NMG_CK_EDGEUSE(eu);
	    cnt++;
            dist = DIST_PNT_PLANE(eu->vu_p->v_p->vg_p->coord, f2_pl);
	...
	for (BU_LIST_FOR(lu, loopuse, &fu2->lu_hd)) {
		...
		NMG_CK_EDGEUSE(eu);
		cnt++;
                dist = DIST_PNT_PLANE(eu->vu_p->v_p->vg_p->coord, f1_pl);
    ...
    if (bg_make_pnt_3planes(pt, pl, f1_pl, f2_pl) < 0) {
