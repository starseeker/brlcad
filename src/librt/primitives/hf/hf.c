			...
			const struct bg_tess_tol *ttol)
{
    ...
    rt_hf_line_sink_append(sink, xip->v, RT_PRIMITIVE_LINE_MOVE);

    VJOIN1(start, xip->v, xip->xlen, xip->x);
    rt_hf_line_sink_append(sink, start, RT_PRIMITIVE_LINE_DRAW);
    ...
    rt_hf_line_sink_append(sink, xip->v, RT_PRIMITIVE_LINE_DRAW);
    goal -= 5;
...
#define HF_GET(_p, _x, _y)	((_p)[(_y)*xip->w+(_x)])
	...
	for (x = 0; x < xip->w; x++) {
	    ...
	    rt_hf_line_sink_append(sink, cur, RT_PRIMITIVE_LINE_DRAW);
	    sp++;
	}
	...
	VJOIN1(start, xip->v, xip->ylen, xip->y);
	rt_hf_line_sink_append(sink, start, RT_PRIMITIVE_LINE_MOVE);
	...
	if (xip->shorts) {
	    ...
	    VJOIN2(cur, start, x, xbasis, *sp, zbasis);
	    rt_hf_line_sink_append(sink, cur, RT_PRIMITIVE_LINE_MOVE);
	    x += half_step;
	    sp = &HF_GET((unsigned short *)xip->mp->apbuf, x, y);
	    for (; x < xip->w; x += step) {
		VJOIN2(cur, start, x, xbasis, *sp, zbasis);
