/*                G E D _ D R A W _ S O U R C E . C P P
 * BRL-CAD
 *
...
_ged_draw_view_export_polygon_record_cb(
	ged_polygon_ref UNUSED(ref),
	const struct ged_polygon_record *polygon,
	void *userdata)
{
	...
	int *keep_going)
{
    ...
    ctx.keep_going = 1;
    ged_polygon_visit_records(view_ctx,
...
static ged_scene_node_ref
ged_draw_obol_scene_node_ref(
    const struct ged_draw_obol_context_token *token)
{
    ged_scene_node_ref ref = GED_SCENE_NODE_REF_NULL;
    if (!token || token->magic != GED_DRAW_OBOL_CONTEXT_TOKEN_MAGIC ||
	!token->gedp || !token->gedp->i || !token->gedp->i->ged_gdp)
	return ref;

    ref.owner = token->gedp->i->ged_gdp->gd_obol_context_owner;
    ref.id = token->token;
    ref.generation = token->generation;
    ref.scene_revision = ged_draw_scene_revision(token->gedp);
    return ref;
}


static struct ged_draw_obol_context_token *
ged_draw_obol_context_from_node_ref(struct ged *gedp,
	ged_scene_node_ref ref)
{
    if (!gedp || !gedp->i || !gedp->i->ged_gdp || !ref.owner || !ref.id ||
	!ref.generation)
	return NULL;

    struct ged_drawable *gdp = gedp->i->ged_gdp;
    if (!gdp->gd_obol_context_tokens_init ||
	ref.owner != gdp->gd_obol_context_owner ||
	...
	if (token && token->magic == GED_DRAW_OBOL_CONTEXT_TOKEN_MAGIC &&
		token->gedp == gedp && token->token == ref.id &&
		token->generation == ref.generation)
	    return token;
    }
    return NULL;
}
...
ged_draw_obol_scene_handle_from_node_ref(struct ged *gedp,
	ged_scene_node_ref ref)
{
    struct ged_draw_obol_context_token *token =
	ged_draw_obol_context_from_node_ref(gedp, ref);
    return token ? ged_draw_scene_handle_make((void *)token,
	GED_DRAW_SCENE_BACKEND_OBOL) : ged_draw_scene_handle_null();
}
...
{
    if (ged_scene_node_ref_is_null(a) || ged_scene_node_ref_is_null(b))
	return ged_scene_node_ref_is_null(a) && ged_scene_node_ref_is_null(b);
    return a.owner == b.owner && a.id == b.id &&
	a.generation == b.generation &&
	a.scene_revision == b.scene_revision;
}
	...
	void *userdata)
{
    ...
    ctx->candidate.path = record->database_path;
    ctx->candidate.instance_key = record->instance_key;
    ctx->candidate.draw_mode = record->draw_mode;
    return ctx->cb(&ctx->candidate, ctx->userdata);
}
...
int
ged_draw_source_clear_db_groups_in_scope(struct ged *gedp, struct ged_view_context *view_ctx)
{
    if (!gedp)
	return 0;
...
int
ged_scene_node_display_summary(struct ged *gedp,
	ged_scene_node_ref node,
	struct ged_draw_scene_display_summary *out)
{
    return ged_draw_scene_handle_display_summary(
	    ged_draw_obol_scene_handle_from_node_ref(gedp, node), out);
}
...
{
    struct ged_draw_obol_context_token *token =
	ged_draw_obol_context_from_node_ref(gedp, node);
...
ged_scene_node_source(struct ged *gedp, ged_scene_node_ref node)
{
	...
	if (token->parent_ctx) {
		...
		return ged_draw_obol_scene_node_ref(parent);
	}
	return GED_SCENE_NODE_REF_NULL;
    }
    return GED_SCENE_NODE_REF_NULL;
}
...
{
    return ged_scene_shape_node(gedp, ged_draw_first_shape_ref(gedp));
}
...
{
    return ged_draw_obol_scene_node_ref(
	ged_draw_obol_context_from_scene_ctx(
	    _ged_draw_shape_ref_obol_context(gedp, ref)));
}
...
ged_scene_shape_cache_node(struct ged *gedp, ged_draw_shape_ref ref)
{
    ...
    return ged_draw_obol_scene_node_ref(token);
}

static ged_draw_stale_reason
ged_bobol_record_stale_reason(const BObolDatabaseSourceSummary &summary)
{
    if (!summary.stale)
...
static int
ged_bobol_database_source_record_summary_for_path_mode(
    struct ged *gedp,
    const char *path,
    int draw_mode,
    struct ged_draw_shape_record_summary *out)
{
    ...
    const char *target_path = ged_draw_dbpath_skip_lead_slash(path);
    SoBRLDatabaseSource *source = NULL;
    BObolDatabaseSourceSummary summary;
    BObolDatabaseSourceSummary fallback;
    SoBRLDatabaseSource *fallback_source = NULL;
    for (int i = 0; i < scene->getDatabaseSourceCount(); i++) {
	SoBRLDatabaseSource *candidate = scene->getDatabaseSource(i);
	BObolDatabaseSourceSummary candidate_summary;
	if (!candidate || !candidate->getSummary(candidate_summary) ||
	    !candidate_summary.valid)
	    continue;
	const char *candidate_path = ged_draw_dbpath_skip_lead_slash(
	    candidate_summary.path.getString());
	if (!BU_STR_EQUAL(candidate_path, target_path))
	    continue;
	if (!fallback_source) {
	    fallback_source = candidate;
	    fallback = candidate_summary;
	}
	if (candidate_summary.representationMode != draw_mode)
	    continue;
	source = candidate;
    ...
    out->stale_reason = ged_draw_source_stale_reason_name(
	ged_bobol_record_stale_reason(summary));
	...
	summary.representationMode : draw_mode;
	...
	VSET(out->center, center[0], center[1], center[2]);
    }
    return 1;
}
				  ...
				  struct ged_draw_shape_record_summary *out)
{
	...
	const int source_mode = token->draw_mode_valid ? token->draw_mode :
	    GED_DRAW_MODE_WIRE;
...
ged_scene_group_node(struct ged *gedp, ged_draw_group_ref ref)
{
    return ged_draw_obol_scene_node_ref(
	ged_draw_obol_context_from_scene_ctx(
	    ged_draw_group_ref_obol_context(gedp, ref)));
}
...
ged_scene_shape_ref(struct ged *gedp, ged_scene_node_ref node)
{
    return ged_draw_registry_shape_ref_from_source_ref(gedp,
	    ged_draw_obol_scene_handle_from_node_ref(gedp, node));
}
			      ...
			      struct db_full_path *out)
{
    ...
    if (ged_scene_group_is_overlay(gedp, group))
	return -1;

    if (!ged_draw_obol_context_from_node_ref(gedp, group))
	return -1;

    const struct db_full_path *stored = ged_scene_node_fullpath(gedp, group);
...
{
    ged_draw_scene_handle scene_ref =
	ged_draw_obol_scene_handle_from_node_ref(gedp, group);
    const char *path = ged_draw_scene_handle_semantic_path(scene_ref);
    if (gedp && path) {
...
int
ged_scene_node_tree_summary(struct ged *gedp,
	ged_scene_node_ref node,
	struct ged_draw_scene_tree_summary *out)
{
...
ged_scene_node_child_at(struct ged *gedp, ged_scene_node_ref node, size_t index)
{
    ...
    void *obol_child = ged_draw_obol_scene_context_child_at(scene_ref,
	    (void *)parent, index);
    return ged_draw_obol_scene_node_ref(
	ged_draw_obol_context_from_scene_ctx(obol_child));
}
...
{
    return ged_draw_scene_handle_name(
	ged_draw_obol_scene_handle_from_node_ref(gedp, node));
}
...
int
ged_scene_node_subtree_bounds(struct ged *gedp,
	ged_scene_node_ref node,
	vect_t *min,
	vect_t *max,
	int include_overlays)
{
    return ged_draw_scene_handle_subtree_bounds(
	    ged_draw_obol_scene_handle_from_node_ref(gedp, node), min, max,
	    include_overlays);
}
...
int
ged_draw_source_snapshot(struct ged *gedp,
				   ged_draw_shape_ref ref,
    ...
    const struct bn_tol *tol)
{
    ...
    if (ged_view_feature_ref_is_null(ref) || !ip)
	return 0;
    ...
    ret = ged_view_feature_points_replace(ref,
	    GED_VIEW_FEATURE_TRANSIENT_PREVIEW,
...
void
ged_draw_overlay_erase_name_context(struct ged *gedp, struct ged_view_context *view_ctx,
				    const char *name)
{
    if (!view_ctx || !name)
	return;
...
int
ged_scene_node_line_summary(struct ged *gedp,
	ged_scene_node_ref node,
	struct ged_draw_view_line_summary *out)
{
    struct _ged_draw_obol_line_summary_ctx ctx = {out};
    if (ged_draw_shape_context_obol_path_apply(gedp, node,
	    _ged_draw_obol_line_summary_cb, &ctx,
...
int
ged_scene_node_line_point_at(struct ged *gedp,
	ged_scene_node_ref node,
	size_t index,
	point_t out)
{
...
int
ged_scene_node_line_command_at(struct ged *gedp,
	ged_scene_node_ref node,
	size_t index,
	int *out)
{
...
int
ged_scene_node_geometry_summary(struct ged *gedp,
	ged_scene_node_ref node,
	struct ged_draw_shape_geometry_summary *out)
{
    struct _ged_draw_obol_geometry_summary_ctx ctx = {out, 0,
...
static int
ged_draw_source_view_context_scene_root_set(struct ged_view_context *view_ctx,
