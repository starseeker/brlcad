...
ged_init(struct ged *gedp)
{
    ...
    void *default_view = rt_view_context_create_with_set_bsg(&gedp->ged_views);
    gedp->ged_gvp = static_cast<decltype(gedp->ged_gvp)>(default_view);
    rt_view_context_name_set_bsg(default_view, "default");
    rt_view_set_context_add_bsg(&gedp->ged_views, default_view);
    bu_ptbl_ins(&gedp->ged_free_views, (long *)default_view);
    ...
    gedp->ged_line_layer_overlay_handler = NULL;
    gedp->ged_line_layer_overlay_data = NULL;
...
void
ged_diagnostic_line_layer_handler_set(struct ged *gedp,
	ged_diagnostic_line_layer_func_t handler,
	void *data)
{
    if (!gedp)
	return;
    gedp->ged_line_layer_overlay_handler = handler;
    gedp->ged_line_layer_overlay_data = data;
}

int
ged_diagnostic_line_layer_handler_available(const struct ged *gedp)
{
    return (gedp && gedp->ged_line_layer_overlay_handler) ? 1 : 0;
}

int
ged_diagnostic_line_layer_publish(struct ged *gedp,
	const char *name,
	const struct bg_line_layer_builder *builder)
{
    if (!gedp || !gedp->ged_line_layer_overlay_handler || !name || !builder)
	return 0;
    return (*gedp->ged_line_layer_overlay_handler)(gedp, name, builder,
	    gedp->ged_line_layer_overlay_data) ? 1 : 0;
}

void
ged_diagnostic_hud_label_handler_set(struct ged *gedp,
	ged_diagnostic_hud_label_func_t handler,
	void *data)
{
    if (!gedp)
	return;
    gedp->ged_hud_label_overlay_handler = handler;
    gedp->ged_hud_label_overlay_data = data;
}

int
ged_diagnostic_hud_label_handler_available(const struct ged *gedp)
{
    return (gedp && gedp->ged_hud_label_overlay_handler) ? 1 : 0;
}

int
ged_diagnostic_hud_label_publish(struct ged *gedp,
	const struct ged_diagnostic_hud_label *label)
{
    if (!gedp || !gedp->ged_hud_label_overlay_handler || !label ||
	    !label->label_id || !label->label_id[0])
	return 0;
    return (*gedp->ged_hud_label_overlay_handler)(gedp, label,
	    gedp->ged_hud_label_overlay_data) ? 1 : 0;
}
...
ged_free(struct ged *gedp)
{
	...
	void *view_ctx = (void *)BU_PTBL_GET(&gedp->ged_free_views, i);
...
{
    if (gedp)
	gedp->ged_gvp = static_cast<decltype(gedp->ged_gvp)>(view_ctx);
}
...
{
    return rt_view_context_scale_set_bsg(view_ctx, scale);
}

extern "C" GED_EXPORT int
ged_view_context_update(void *view_ctx)
{
    return rt_view_context_update_bsg(view_ctx);
}

extern "C" GED_EXPORT int
ged_view_context_is_independent(const void *view_ctx)
{
    return rt_view_context_is_independent_bsg(view_ctx);
}

extern "C" GED_EXPORT int
ged_view_context_independent_scope_is_null(void *view_ctx, int create)
{
    return rt_view_context_independent_scope_is_null_bsg(view_ctx, create);
}

extern "C" GED_EXPORT void
ged_view_context_independent_scope_destroy(void *view_ctx)
{
    rt_view_context_independent_scope_destroy_bsg(view_ctx);
}

static int
ged_view_clear_flags_to_bsg(int flags)
{
    int bsg_flags = 0;
...
{
    return rt_view_context_clear_bsg(view_ctx,
	    ged_view_clear_flags_to_bsg(flags));
}

extern "C" GED_EXPORT int
ged_view_context_cleared_set(void *view_ctx, int cleared)
{
    return rt_view_context_cleared_set_bsg(view_ctx, cleared);
}

extern "C" GED_EXPORT int
ged_view_context_refresh_drawn_count_get(const void *view_ctx)
{
    return rt_view_context_refresh_drawn_count_from_bsg(view_ctx);
}

extern "C" GED_EXPORT int
ged_view_context_refresh_request(void *view_ctx, uint32_t flags)
{
    return rt_view_context_refresh_request_bsg(view_ctx, flags);
}
...
{
    return rt_view_context_create_copy_with_set_bsg(src_view_ctx, view_set_ctx);
}
...
ged_view_context_update_callback_set(void *view_ctx,
	...
	void *data)
{
    return rt_view_context_update_callback_set_bsg(view_ctx,
	    reinterpret_cast<rt_view_update_callback_bsg_t>(callback), data);
}

extern "C" GED_EXPORT void *
ged_view_context_display_manager_get(const void *view_ctx)
{
    return rt_view_context_display_manager_from_bsg(view_ctx);
}

extern "C" GED_EXPORT int
ged_view_context_display_manager_set(void *view_ctx, void *dmp)
{
    return rt_view_context_display_manager_set_bsg(view_ctx, dmp);
}

extern "C" GED_EXPORT int
ged_view_context_width_get(const void *view_ctx)
{
    return rt_view_context_width_from_bsg(view_ctx);
}

extern "C" GED_EXPORT int
ged_view_context_height_get(const void *view_ctx)
{
    return rt_view_context_height_from_bsg(view_ctx);
}

extern "C" GED_EXPORT int
ged_view_context_dimensions_set(void *view_ctx, int width, int height)
{
    return rt_view_context_dimensions_set_bsg(view_ctx, width, height);
}

extern "C" GED_EXPORT fastf_t *
ged_view_context_scale_storage_get(void *view_ctx)
{
    return rt_view_context_scale_storage_from_bsg(view_ctx);
}

extern "C" GED_EXPORT int
ged_view_context_framebuffer_mode_get(const void *view_ctx)
{
    return rt_view_context_framebuffer_mode_from_bsg(view_ctx);
}

extern "C" GED_EXPORT int
ged_view_context_framebuffer_mode_set(void *view_ctx, int mode)
{
    return rt_view_context_framebuffer_mode_set_bsg(view_ctx, mode);
}
...
{
    return rt_view_context_perspective_set_bsg(view_ctx, perspective);
}

extern "C" GED_EXPORT fastf_t
ged_view_context_size_get(const void *view_ctx)
{
    return rt_view_context_size_from_bsg(view_ctx);
}

extern "C" GED_EXPORT int
ged_view_context_size_set(void *view_ctx, fastf_t size)
{
    return rt_view_context_size_set_bsg(view_ctx, size);
}

extern "C" GED_EXPORT fastf_t
ged_view_context_inverse_size_get(const void *view_ctx)
{
    return rt_view_context_inverse_size_from_bsg(view_ctx);
}
...
{
    return rt_view_context_view2model_set_bsg(view_ctx, view2model);
}

extern "C" GED_EXPORT void
ged_view_context_pmodel2view_get(mat_t pmodel2view, const void *view_ctx)
{
    rt_view_context_pmodel2view_from_bsg(pmodel2view, view_ctx);
}
...
{
    return rt_view_context_knobs_translate_bsg(view_ctx, tvec, model_flag);
}
...
{
    return rt_view_context_knobs_rotate_bsg(view_ctx, rvec, origin, coords,
	    obj_rot, pvt_pt);
}
...
ged_view_context_interactive_rect_state_get(struct rt_view_interactive_rect_state *rect,
	const void *view_ctx)
{
    return rt_view_context_interactive_rect_state_from_bsg(rect, view_ctx);
}
...
ged_rt_fb_refresh(struct ged *gedp)
{
    ...
    GED_CK_MAGIC(gedp);
    struct dm *dmp = (struct dm *)ged_view_context_display_manager_get(gedp->ged_gvp);
    if (!dmp)
	return;
