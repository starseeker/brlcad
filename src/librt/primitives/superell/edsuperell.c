...
static void
superell_ed(struct rt_edit *s, int arg, int UNUSED(a), int UNUSED(b), void *UNUSED(data))
{
    rt_edit_superell_set_edit_mode(s, arg);
}

struct rt_edit_menu_item superell_menu[] = {
...
rt_edit_superell_edit(struct rt_edit *s)
{
	...
	case ECMD_SUPERELL_SCALE_C:
	case ECMD_SUPERELL_SCALE_ABC:
	    return rt_edit_superell_pscale(s);
	default:
