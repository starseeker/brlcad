...
set(_test_dir "${BRLCAD_ROOT}/Testing/Temporary/qged_view_settings_${_suffix}")
set(_database "${_test_dir}/settings.g")
set(_setup "${_test_dir}/setup.mged")
file(MAKE_DIRECTORY "${_test_dir}")
file(WRITE "${_setup}"
  "put settings_box.s bot mode volume orient rh flags {} V {{-2 -2 -1} {2 -2 -1} {2 2 -1} {-2 2 -1} {-2 -2 1} {2 -2 1} {2 2 1} {-2 2 1}} F {{0 2 1} {0 3 2} {4 5 6} {4 6 7} {0 1 5} {0 5 4} {1 2 6} {1 6 5} {2 3 7} {2 7 6} {3 0 4} {3 4 7}}\n"
  "r settings.r u settings_box.s\n"
  "q\n")
...
if(NOT _setup_result EQUAL 0)
  ...
  message(FATAL_ERROR "view-settings fixture setup failed: ${_setup_error}")
endif()
...
if(NOT _qged_result EQUAL 0)
  file(REMOVE_RECURSE "${_test_dir}")
  message(FATAL_ERROR
    "qged view-settings replay failed (${_qged_result})\n"
    "stdout:\n${_qged_output}\n"
    "stderr:\n${_qged_error}")
endif()

foreach(_image
    view-settings-base.png
    view-settings-cut.png
    view-settings-restored.png)
  if(NOT EXISTS "${_run_dir}/${_image}")
    file(REMOVE_RECURSE "${_test_dir}")
    message(FATAL_ERROR "qged view-settings replay did not create ${_image}")
  endif()
endforeach()
file(REMOVE_RECURSE "${_test_dir}")
