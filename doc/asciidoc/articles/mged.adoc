...
gui [_-config b|c|g_] [__-d display_string__] [__-gd graphics_display_string__] [__-host host_type__] [_-id name_] [_-c -h -j -s_]::
...
----
       ...
       -host host_type         Selects the Obol host used for graphics windows.
                               tkobol is the supported host.
...
The “oed” command places MGED directly into matrix edit mode. With one path, __path_lhs__ is the complete path to the object being edited; the matrix immediately above its final component is selected and the object's bounding-box center is used as the keypoint. With two paths, __path_rhs__ must end in a primitive shape, and __path_lhs__ must identify a combination containing the first component of __path_rhs__. The matrix between the final component of __path_lhs__ and the first component of __path_rhs__ is selected, and the final primitive in __path_rhs__ supplies the keypoint. The selected object path must currently be displayed in MGED.
...
– Place MGED into matrix edit mode for _region1_, using the center of its bounding box as the keypoint.
...
The “ps” command exports the retained current view to the specified __output_file__ in _PostScript_ format. The _-f_ option allows the _font_ to be user-specified. The _-t_ option allows the user to provide a title (the default is “No Title”). The _-c_ option allows the user to specify the creator of the file. The _s_ specifies the size of the drawing in inches. The _l_ specifies the width of the lines drawn.
...
Axes model_draw=0 model_size=500 model_linewidth=1 model_pos=0,0,0 model_tick_enable=0 model_tick_interval=100 model_ticks_per_major=10 model_tick_length=4 model_tick_major_length=8 model_tick_threshold=8 view_draw=0 view_size=500 view_linewidth=1 view_pos=0,0 edit_draw=0 edit_size1=500 edit_size2=500 edit_linewidth1=1 edit_linewidth2=1.

–Prints the values of the axes

mged *rset ax model_size* – Get size of model axes 500.

mged *rset ax model_tick_enable* – Enable (1) or disable (0) a ticked scale
along the model axes. The tick spacing is set with *rset ax
model_tick_interval* (in mm), with a major tick every *rset ax
model_ticks_per_major* ticks. This gives a measurable scale in the target
coordinate system.
...
The “share” command provides a mechanism to share (or unshare with the *u* option) resources among display managers. When a resource is shared between two or more display managers, any change to that resource is seen only in the sharing display managers. The supported resource types are: ad (ADC), ax (Axes), c (Color Schemes), g (Grid), m (Menu), r (Rubber Band), vi (View), and var (MGED Variables).
...
*mged_default(host_type)*

__mged_default(host_type)__ indicates the default Obol GUI host type.
