...
CONSTANT MaxViewEpoch, MaxVisibilityEpoch, MaxPolicyEpoch
...
RevisionStamp(inventory, available, visibility, view, policy, capacity) ==
    [inventory |-> inventory,
...
vars == <<workload, viewEpoch, visibilityRevision, inputOpen,
          discovered, profileCertified,
...
CurrentRevisionStamp ==
    RevisionStamp(inventoryRevision, availabilityRevision,
                  visibilityRevision, viewEpoch, policyRevision,
                  capacityRevision)
...
TypeOK ==
    ...
    /\ visibilityRevision \in 0..MaxVisibilityEpoch
    ...
    /\ plannedRevisionStamp \in
         [inventory: Nat, availability: Nat, visibility: Nat, view: Nat,
...
Init ==
    ...
    /\ policyRevision = 0
    /\ capacityRevision = 1
    /\ plannedRevisionStamp = RevisionStamp(0, 0, 0, 0, 0, 0)
    /\ planningRevisionStamp = RevisionStamp(0, 0, 0, 0, 0, 0)
    /\ planning = FALSE
    /\ considered = {}
...
Discover(occurrence) ==
    ...
    /\ UNCHANGED <<workload, viewEpoch, visibilityRevision, inputOpen,
                    profileCertified,
...
BeginInput ==
    ...
    /\ inputOpen' = TRUE
    /\ capacityMeasured' = FALSE
    /\ UNCHANGED <<workload, visibilityRevision, discovered,
                    profileCertified, availability,
...
EndInput ==
    /\ inputOpen
    /\ inputOpen' = FALSE
    /\ UNCHANGED <<workload, viewEpoch, visibilityRevision, discovered,
                    profileCertified,
                    availability, enrichmentExhausted, workingAsset,
                    workGoal, committedRep, committedQuality, committedKind,
                    candidateRep, candidateQuality, candidateKind,
...
\* Exact visibility/classification is an independent semantic input.  It may
\* invalidate an in-progress plan, but it must not erase a capacity result or
\* impersonate a camera epoch.  This is the sixth evidence domain carried by
\* production planning and publication records.
ChangeVisibility ==
    /\ ~inputOpen
    /\ ~framePending
    /\ visibilityRevision < MaxVisibilityEpoch
...
ActivePipelineHasInternalOrExternalWitness ==
    Outcome = "active" =>
        \/ inputOpen
