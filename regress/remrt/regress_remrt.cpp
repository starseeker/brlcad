...
/*
 * Compare two raw .pix (RGB) files with a one-channel-value tolerance.
 * Returns 0 when every channel is within that tolerance, and nonzero when
 * one or more pixels exceed it.
 *
		     ...
		     const std::string &file_b)
{
    ...
    /* IPC delivery can alter the final floating-point rounding of pixels at
     * shadow edges even when the reconstructed view is equivalent.  Keep
     * reporting those differences, but only fail for a visible discrepancy. */
