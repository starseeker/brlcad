...
Note_to_vlist(size_t entno, struct bu_list *vhead)
{
	...
	if (internal_rot) {
		...
		bg_vlist_2string(vhead, &free_hd, one_char ,
				 tmp_x, tmp_y, local_scale,
				 (double)(rot_ang*RAD2DEG));
	    }
	} else
	    bg_vlist_2string(vhead, &free_hd, str ,
...
Curve_to_vlist(struct bu_list *vhead, struct ptlist *ptlist, struct bu_list *vlfree)
{
    ...
    BG_ADD_VLIST(vlfree, vhead, ptr->pt, BG_VLIST_LINE_MOVE);

    ptr = ptr->next;
    while (ptr != NULL) {
	BG_ADD_VLIST(vlfree, vhead, ptr->pt, BG_VLIST_LINE_DRAW);
	ptr = ptr->next;
    }
}
...
Leader_to_vlist(size_t entno, struct bu_list *vhead, struct bu_list *vlfree)
{
    ...
    BG_ADD_VLIST(vlfree, vhead, tmp2, BG_VLIST_LINE_MOVE);
    MAT4X3PNT(tmp, *dir[entno]->rot, v2);
    BG_ADD_VLIST(vlfree, vhead, tmp, BG_VLIST_LINE_DRAW);
	...
	case 6: {
		...
		BG_ADD_VLIST(vlfree, vhead, tmp2, BG_VLIST_LINE_DRAW);
	    }
	}
	    break;
	case 7:
	case 8:
	    ...
	    BG_ADD_VLIST(vlfree, vhead, tmp3, BG_VLIST_LINE_DRAW);
	    tmp[0] += v3[0]*a;
	    tmp[1] += v3[1]*a;
	    MAT4X3PNT(tmp3, *dir[entno]->rot, tmp);
	    BG_ADD_VLIST(vlfree, vhead, tmp3, BG_VLIST_LINE_DRAW);
	...
	case 10:
	    ...
	    tmp[2] = v1[2];
	    MAT4X3PNT(tmp3, *dir[entno]->rot, tmp);
	    BG_ADD_VLIST(vlfree, vhead, tmp3, BG_VLIST_LINE_MOVE);
	    tmp[0] -= v3[0]*a + v2[0]*b;
	    tmp[1] -= v3[1]*a + v2[1]*b;
	    MAT4X3PNT(tmp3, *dir[entno]->rot, tmp);
	    BG_ADD_VLIST(vlfree, vhead, tmp3, BG_VLIST_LINE_DRAW);
