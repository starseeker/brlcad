...
*/
	...
	if (vert_edge_status[bot->faces[j*3+0]] &&  vert_edge_status[bot->faces[j*3+1]]) {
	    (void)primitive_lod_line_set_append(realization, &(bot->vertices[bot->faces[j*3+0]*3]), RT_PRIMITIVE_LINE_MOVE);
	    (void)primitive_lod_line_set_append(realization, &(bot->vertices[bot->faces[j*3+1]*3]), RT_PRIMITIVE_LINE_DRAW);
	}
	if (vert_edge_status[bot->faces[j*3+1]] &&  vert_edge_status[bot->faces[j*3+2]]) {
	    (void)primitive_lod_line_set_append(realization, &(bot->vertices[bot->faces[j*3+1]*3]), RT_PRIMITIVE_LINE_MOVE);
	    (void)primitive_lod_line_set_append(realization, &(bot->vertices[bot->faces[j*3+2]*3]), RT_PRIMITIVE_LINE_DRAW);
	}
	if (vert_edge_status[bot->faces[j*3+2]] &&  vert_edge_status[bot->faces[j*3+0]]) {
	    (void)primitive_lod_line_set_append(realization, &(bot->vertices[bot->faces[j*3+2]*3]), RT_PRIMITIVE_LINE_MOVE);
	    (void)primitive_lod_line_set_append(realization, &(bot->vertices[bot->faces[j*3+0]*3]), RT_PRIMITIVE_LINE_DRAW);
	}
    }
