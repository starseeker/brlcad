...
set mged_helpdevel_data(get_display_list)	{{}	{returns a list of all active graphics views}}
...
set mged_helpdevel_data(share)		{{[-u] resource view1 [view2]}	{Provides a mechanism to (un)share resources among graphics views.}}
...
set mged_helpdevel_data(tie)		{{[[-u] cw [view]]}	{ties/associates a command window (cw) with a graphics view}}
set mged_helpdevel_data(view_ring)	{{cmd [i]}	{manipulates the view_ring for the current graphics view}}
...
set mged_helpdevel_data(view2model_lu)	{{vx vy vz}	{given a point in view coordinates (local units),
    convert it to model coordinates (local units).}}
set mged_helpdevel_data(winset)		{{[pathname]}	{sets the current graphics view to pathname}}
