	...
	unsigned long rgb)
{
    ...
    (void)controller->features().removePrefix(name,
    ...
    const auto add_metadata = [&metadata](const char *key, const char *value) {
	BObolFeatureMetadata item;
	item.key = key;
	item.value = value;
	metadata.push_back(item);
    };
    add_metadata("result.feature", name);
    add_metadata("result.format", result_format);
    add_metadata("result.point_count", point_count);
    add_metadata("result.owner", "vdraw");
    add_metadata("result.kind", result_kind);
    add_metadata("result.index_count", index_count);

    std::vector<SbVec3f> points;
    points.reserve(pg->geometry.point_count);
    for (size_t i = 0; i < pg->geometry.point_count; i++)
	points.push_back(SbVec3f(static_cast<float>(pg->points[i][X]),
	    static_cast<float>(pg->points[i][Y]),
	    static_cast<float>(pg->points[i][Z])));
	...
	case GED_DRAW_OVERLAY_GEOMETRY_LINE_SET: {
	    ...
	    commands.reserve(pg->geometry.command_count);
	    for (size_t i = 0; i < pg->geometry.command_count; i++)
		commands.push_back(static_cast<int32_t>(pg->commands[i]));
	    handle = controller->features().publishLineSet(name,
		BObolFeatureScope::Shared, points, commands, &style, &owner);
	    break;
	}
	case GED_DRAW_OVERLAY_GEOMETRY_POINT_SET:
	    handle = controller->features().publishPointSet(name,
		BObolFeatureScope::Shared, points, &style, &owner);
	    break;
	case GED_DRAW_OVERLAY_GEOMETRY_INDEXED_FACE_SET: {
	    std::vector<SbVec3f> normals;
	    normals.reserve(pg->geometry.normal_count);
	    for (size_t i = 0; i < pg->geometry.normal_count; i++)
		normals.push_back(SbVec3f(static_cast<float>(pg->normals[i][X]),
		    static_cast<float>(pg->normals[i][Y]),
		    static_cast<float>(pg->normals[i][Z])));
	    std::vector<int32_t> indices;
	    indices.reserve(pg->geometry.index_count);
	    for (size_t i = 0; i < pg->geometry.index_count; i++)
		indices.push_back(static_cast<int32_t>(pg->indices[i]));
	    handle = controller->features().publishIndexedFaceSet(name,
		BObolFeatureScope::Shared, points, normals, indices, &style,
		&owner);
	    break;
	}
	default:
    ...
    overlay.sourcePath = name;
    return controller->features().setOverlayInfo(handle, overlay) ? 1 : -1;
}
