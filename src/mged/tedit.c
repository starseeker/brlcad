...
writesolid(struct mged_state *s)
{
	...
	default:
	    ...
	    return 1;
	case ID_TOR:
	    tor = (struct rt_tor_internal *)MEDIT(s)->es_int.idb_ptr;
	...
	case ID_TGC:
	case ID_REC:
	    tgc = (struct rt_tgc_internal *)MEDIT(s)->es_int.idb_ptr;
	...
	case ID_ELL:
	case ID_SPH:
	    ell = (struct rt_ell_internal *)MEDIT(s)->es_int.idb_ptr;
	...
	case ID_ARB8:
	    ...
	    break;
	case ID_HALF:
	    haf = (struct rt_half_internal *)MEDIT(s)->es_int.idb_ptr;
	    fprintf(fp, "Plane: %.9f %.9f %.9f %.9f%s", V4BASE2LOCAL(haf->eqn), eol);
	    break;
	case ID_GRIP:
	    grip = (struct rt_grip_internal *)MEDIT(s)->es_int.idb_ptr;
	    ...
	    break;
	case ID_PARTICLE:
	    part = (struct rt_part_internal *)MEDIT(s)->es_int.idb_ptr;
	    ...
	    break;
	case ID_RPC:
	    rpc = (struct rt_rpc_internal *)MEDIT(s)->es_int.idb_ptr;
	    ...
	    break;
	case ID_RHC:
	    rhc = (struct rt_rhc_internal *)MEDIT(s)->es_int.idb_ptr;
	    ...
	    break;
	case ID_EPA:
	    epa = (struct rt_epa_internal *)MEDIT(s)->es_int.idb_ptr;
	    ...
	    break;
	case ID_EHY:
	    ehy = (struct rt_ehy_internal *)MEDIT(s)->es_int.idb_ptr;
	    ...
	    break;
	case ID_HYP:
	    hyp = (struct rt_hyp_internal *)MEDIT(s)->es_int.idb_ptr;
	    ...
	    break;
	case ID_ETO:
	    eto = (struct rt_eto_internal *)MEDIT(s)->es_int.idb_ptr;
	    ...
	    break;
	case ID_SUPERELL:
	    superell = (struct rt_superell_internal *)MEDIT(s)->es_int.idb_ptr;
	    ...
	    break;
	case ID_DATUM:
	    datum = (struct rt_datum_internal *)MEDIT(s)->es_int.idb_ptr;
	    do {
		if (!ZERO(datum->w))
