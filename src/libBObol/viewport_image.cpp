...
SoBRLViewportImage::rebuildGeometry(void)
{
    ...
    if (bobol_image_payload_load(source, &payload) != 0)
	return -1;

    float displayWidth = 0.0f;
    float displayHeight = 0.0f;
    SbVec2f requested = this->size.getValue();
    bobol_image_fit_size((float)payload.width, (float)payload.height,
    ...
    float u1 = 1.0f;
    float v1 = 1.0f;
    bobol_image_texture_rect((float)payload.width, (float)payload.height,
			       this->sourceCenter.getValue(), this->sourceZoom.getValue(),
			       this->fit.getValue(), &u0, &v0, &u1, &v1);

    SoTexture2 *texture = NULL;
    SoFaceSet *face = NULL;
    SoSeparator *quad = bobol_image_make_textured_quad(&payload,
