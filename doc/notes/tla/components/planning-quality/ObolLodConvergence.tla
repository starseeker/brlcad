...
\* Workload profiles used only to select finite protocol ranks.
Profiles == {"lucy", "scene"}
...
CompleteFrame ==
    /\ framePending
    /\ framePending' = FALSE
    \* Completion publishes an owner-thread cursor even at the end of the
    \* final bounded window.  A non-empty cursor dispatches its successor;
    \* an empty cursor must be retired explicitly by RetireEmptySubmission.
    \* Computing the boolean directly from MoreOwnerWork made that important
    \* empty-tail transition unreachable and concealed the historical
    \* no-producer loop from TLC coverage.
    /\ submissionPending' = TRUE
...
\* A submission level is a bounded cursor, not an abstract desire.  At a
\* quiet boundary it must have a legal next owner-thread action.  While input
\* is open, closing or superseding that environment session is the explicit
\* external witness; the production scheduler is not assumed to manufacture
\* a quiet allocation transition during camera motion.
PendingSubmissionHasWitness ==
    submissionPending => inputOpen \/ SubmissionCanProgress
...
Stable == inputEpoch = MaxInputEpoch /\ Settled
...
DeadlockOnlyAtTerminal == ~ENABLED <<Next>>_vars => Settled
...
\* User input is an environment transition, not a production progress
\* witness.  At any quiet boundary the controller must either settle that
\* epoch or be interrupted by a later input; the model does not assume that a
\* user will open or close enough sessions to reach MaxInputEpoch.
EventuallySettledOrInterrupted ==
    [](~inputOpen => <>(Settled \/ inputOpen))
