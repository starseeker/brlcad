		       ...
		       struct vol_line_sink *sink)
{
    ...
    rt_vol_line_sink_append(sink, arot, RT_PRIMITIVE_LINE_MOVE);

    VELMUL(s, vip->cellsize, b);
    MAT4X3PNT(prot, mat, s);
    rt_vol_line_sink_append(sink, prot, RT_PRIMITIVE_LINE_DRAW);
    ...
    rt_vol_line_sink_append(sink, arot, RT_PRIMITIVE_LINE_DRAW);
}
