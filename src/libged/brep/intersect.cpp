...
int
brep_intersect_surface_surface(struct rt_db_internal *intern1, struct rt_db_internal *intern2, int i, int j, struct bg_line_layer_builder *plot)
{
    RT_CK_DB_INTERNAL(intern1);
    RT_CK_DB_INTERNAL(intern2);
    struct rt_brep_internal *bi1, *bi2;
    ...
    plotsurface(surf1, plot, 100, 10, PURERED);
    plotsurface(surf2, plot, 100, 10, BLUE);
	...
	switch (events[k].m_type) {
		...
		plotcurveonsurface(events[k].m_curveA, &surf1, plot, 1000, PEACH);
		plotcurveonsurface(events[k].m_curveB, &surf2, plot, 1000, DARKVIOLET);
		plotcurve(*(events[k].m_curve3d), plot, 1000, GREEN);
		break;
	    case ON_SSX_EVENT::ssx_tangent_point:
	    case ON_SSX_EVENT::ssx_transverse_point:
		plotpoint(surf1.PointAt(events[k].m_pointA.x, events[k].m_pointB.y), plot, PEACH);
		plotpoint(surf2.PointAt(events[k].m_pointB.x, events[k].m_pointB.y), plot, DARKVIOLET);
		plotpoint(events[k].m_point3d, plot, GREEN);
		break;
	    default:
		break;
	}
