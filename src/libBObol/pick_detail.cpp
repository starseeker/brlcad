...
BObolSourceMeshPickResult::BObolSourceMeshPickResult(void)
{
    clear();
}

void
BObolSourceMeshPickResult::clear(void)
{
    detail.clear();
...
BObolRtPickResult::BObolRtPickResult(void)
{
    clear();
}

void
BObolRtPickResult::clear(void)
{
    detail.clear();
    point = SbVec3f(0.0f, 0.0f, 0.0f);
...
static int
pick_source_mesh_face_index(const BObolLodMeshPayload &mesh,
...
static int
pick_source_mesh_vertex_index(const BObolLodMeshPayload &mesh,
			      int localIndex)
{
    if (localIndex < 0)
	return localIndex;
...
pick_clip_ray_interval(const struct application *ap,
	const bobol_rt_pick_state *state, double inputMinimum,
...
static int
bobol_pick_rt_hit(struct application *ap,
		    ...
		    struct seg *UNUSED(segp))
{
    ...
    bobol_rt_pick_state *state =
	static_cast<bobol_rt_pick_state *>(ap->a_uptr);
...
static int
bobol_pick_rt_miss(struct application *UNUSED(ap))
{
    return 0;
}
...
BObolRtPickCache::BObolRtPickCache(void) :
    rtip(NULL),
    database(NULL),
    ready(FALSE)
{
...
SbBool
BObolRtPickCache::prepare(struct db_i *dbip,
...
SbBool
BObolRtPickCache::isReady(void) const
{
    return this->ready && this->rtip ? TRUE : FALSE;
}

int
BObolRtPickCache::getObjectPathCount(void) const
{
    return static_cast<int>(this->objectPaths.size());
}

SbBool
BObolRtPickCache::pickRay(BObolRtPickResult &pick,
			    ...
			    float minimumDistance) const
{
    ...
    ap.a_hit = bobol_pick_rt_hit;
    ap.a_miss = bobol_pick_rt_miss;
    ap.a_overlap = NULL;
    ap.a_onehit = 0;
    ap.a_rt_i = this->rtip;
	    ap.a_resource = resource;
    ap.a_uptr = &state;
    ap.a_purpose = "BObolRtPickCache::pickRay";
...
SbBool
BObolRtPickCache::initializeResource(struct resource *resource, int cpu) const
{
...
SoBRLSourceMeshPickAction::makeSourceBackedFullDetailLodRequest(int index,
	...
	const BObolLodRequest *templateRequest) const
{
    if (index < 0 ||
    ...
    return bobol_lod_rt_source_full_detail_request_from_source_mesh_request(
	       request,
...
SoBRLSourceMeshPickAction::submitSourceBackedFullDetailRequests(
    BObolLodService *service, uint64_t generation, struct db_i *dbip,
    ...
    const BObolLodRequest *templateRequest) const
{
	...
	for (size_t j = 0; j < results.size(); j++) {
	    if (used[j] ||
		!bobol_lod_result_matches_request(results[j], expected))
		continue;
...
SoBRLSourceMeshPickAction::rayIntersectsBounds(
    const BObolSourceMeshRequest &request) const
{
