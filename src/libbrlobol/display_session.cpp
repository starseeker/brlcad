...
struct brlobol_display_session {
    ...
    const brlobol_display_provider_t *provider = NULL;
    void *provider_instance = NULL;
    BRLObolWindowHost attachment_host;
    imgstream_fb_t *framebuffer = NULL;
    std::atomic<bool> present_pending = false;
};

static std::mutex provider_lock;
static std::vector<const brlobol_display_provider_t *> providers;

static int
session_present(brlobol_display_session_t *session, imgstream_fb_t *fb,
	bool force)
{
    if (!session || !fb)
	return -1;
	...
	return -1;
    return brlobol_display_endpoint_request_frame(session->endpoint,
	"framebuffer-present") ? 0 : -1;
}

static int
session_request_present(brlobol_display_session_t *session)
{
    if (!session)
	return -1;
    bool expected = false;
	...
	std::memory_order_acq_rel))
	return 0;
    if (brlobol_display_endpoint_request_frame(session->endpoint,
	"framebuffer-dirty"))
	return 0;
    session->present_pending.store(false, std::memory_order_release);
    return -1;
	...
	void *data)
{
	...
	static_cast<brlobol_display_session_t *>(data);
    BRLObolViewController *controller = session ?
	static_cast<BRLObolViewController *>(
	    brlobol_display_endpoint_controller(session->endpoint)) : NULL;
...
session_flush(imgstream_fb_t *fb, void *data)
{
    ...
    return session_request_present(static_cast<brlobol_display_session_t *>(data));
}
...
session_poll_rate(const imgstream_fb_t *UNUSED(fb), void *data)
{
	...
	static_cast<const brlobol_display_session_t *>(data);
...
extern "C" int
brlobol_display_provider_register(const brlobol_display_provider_t *provider)
{
    if (!provider || provider->abi_version != BRLOBOL_DISPLAY_PROVIDER_ABI_VERSION ||
	provider->struct_size < sizeof(brlobol_display_provider_t) ||
	!provider->name || !provider->name[0] || !provider->open)
	return 0;
    std::lock_guard<std::mutex> guard(provider_lock);
    for (const brlobol_display_provider_t *candidate : providers) {
	if (std::strcmp(candidate->name, provider->name) == 0)
	    return candidate == provider;
    }
    providers.push_back(provider);
    std::stable_sort(providers.begin(), providers.end(),
	[](const brlobol_display_provider_t *a,
	   const brlobol_display_provider_t *b) {
	    return a->priority > b->priority;
	});
    return 1;
}

extern "C" brlobol_display_session_t *
brlobol_display_session_open(const char *spec, size_t width, size_t height,
	const char *title)
{
    ...
    std::vector<const brlobol_display_provider_t *> candidates;
    {
	std::lock_guard<std::mutex> guard(provider_lock);
	candidates = providers;
    }
    for (const brlobol_display_provider_t *provider : candidates) {
	brlobol_display_endpoint_t *endpoint =
	    brlobol_display_endpoint_create(NULL, BRLOBOL_ENDPOINT_OWN_CONTROLLER);
	if (!endpoint)
	    continue;
	void *provider_instance = NULL;
	...
	brlobol_display_session_t *session = new (std::nothrow)
	    brlobol_display_session_t;
	if (!session) {
	    brlobol_display_endpoint_destroy(endpoint);
	    if (provider->close)
...
extern "C" void
brlobol_display_session_close(brlobol_display_session_t *session)
{
    if (!session)
	return;
    if (session->framebuffer)
    ...
    brlobol_display_endpoint_destroy(session->endpoint);
...
brlobol_display_session_framebuffer(const brlobol_display_session_t *session)
{
    return session ? session->framebuffer : NULL;
}

extern "C" int
brlobol_display_session_present(brlobol_display_session_t *session)
{
...
extern "C" int
brlobol_display_session_poll(brlobol_display_session_t *session)
{
...
extern "C" long
brlobol_display_session_poll_rate(const brlobol_display_session_t *session)
{
    return session ? session_poll_rate(session->framebuffer,
	const_cast<brlobol_display_session_t *>(session)) : 0;
}

extern "C" int
brlobol_display_session_run(brlobol_display_session_t *session,
	brlobol_display_session_task_t task, void *task_data)
{
	...
	(void)brlobol_display_session_poll(session);
	const long rate = std::max(1000L,
	    brlobol_display_session_poll_rate(session));
