...
static char *
_join_path(const char *lhs, const char *rhs)
{
    struct bu_vls path = BU_VLS_INIT_ZERO;
    char *ret;

    bu_vls_sprintf(&path, "%s/%s", lhs, rhs);
	   ...
	   const struct hygiene_rule *rules, size_t rule_count)
{
    ...
    abs = _join_path(srcroot, relroot);
    dirp = opendir(abs);
    if (!dirp) {
	fprintf(stderr, "Unable to open %s: %s\n", abs, strerror(errno));
	g_failures++;
	bu_free(abs, "api hygiene scan path");
	return;
    }
    bu_free(abs, "api hygiene scan path");
	...
	child_rel = _join_path(relroot, dp->d_name);
	child_abs = _join_path(srcroot, child_rel);
	if (stat(child_abs, &sb) != 0) {
	    bu_free(child_abs, "api hygiene child absolute path");
	    bu_free(child_rel, "api hygiene child relative path");
	    continue;
	}
			      ...
			      const char *message)
{
    ...
    abs = _join_path(srcroot, rel);
    fp = fopen(abs, "r");
    if (!fp) {
	fprintf(stderr, "Unable to open %s: %s\n", abs, strerror(errno));
	g_failures++;
	bu_free(abs, "api hygiene exact file path");
	return;
    }
    bu_free(abs, "api hygiene exact file path");
