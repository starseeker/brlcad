...
== 7.44.0
command-line tools
	plot3-dm : obsolete standalone libdm viewer; use the Obol-backed
	`overlay` command in MGED, qged, or GSH
...
mged
	...
	rm obj.c mem1 mem2 -> new behavior: rm deletes unreferenced
        objects from database. Removal is available via:
		comb obj.c rm mem1 mem2
		comb rm -C obj.c mem1 mem2
		rm obj.c/mem1 obj.c/mem2
        [deprecated 7.26]
