...
public:
    PrototypeTiming()
    {
	const char *enabled_env = getenv("BOBOL_PROTOTYPE_TIMING");
	...
	BObolRealizedShapeSummary &summary)
{
	...
	BObolRealizedShapeSummary candidate;
	if (!source->getRealizedShapeSummary(i, candidate) ||
	    candidate.shapeKind != BObolRealizedShapeSummary::SHAPE_MESH ||
			     ...
			     float shaded_rel_tol = 0.15f)
{
    ...
    BObolRealizedShapeSummary wireSummary;
	...
	const SbViewportRegion &viewport)
{
    ...
    BObolRealizedShapeSummary shadedSummary;
				      ...
				      const SbViewportRegion &viewport)
{
    ...
    BObolCompactInstanceHandle pointHandle;
    BObolCompactInstanceSummary pointSummary;
...
{
    bu_setprogname("test_bobol_prototype");
    PrototypeTiming timing;

    bobol_init(NULL);
    PrototypeContextManager prototypeManager;
    bobol_init(&prototypeManager);
    if (SoDB::getContextManager() != &prototypeManager)
	FAIL("bobol_init should update the context manager after null initialization");
	...
	BObolDatabaseSourceSummary publicationSummary;
	...
	BObolExternalTriangleMesh triangleMesh;
	...
	BObolViewController viewController(root, camera);
    ...
    BObolLodRequest meshLodRequest;
    ...
    meshLodRequest.drawMode = BOBOL_LOD_DRAW_SHADED;
    meshLodRequest.providerId = "prototype-provider";
    meshLodRequest.providerVersion = "1";
    meshLodRequest.qualityTier = BOBOL_LOD_QUALITY_PROXY;
    meshLodRequest.bounds = SbBox3f(SbVec3f(0.0f, 0.0f, 0.0f),
				    SbVec3f(2.0f, 2.0f, 1.0f));

    std::vector<BObolLodDependency> meshDependencies;
    BObolLodDependency meshDependency;
    ...
    meshDependency.requiredQualityTier = BOBOL_LOD_QUALITY_PROXY;
    meshDependency.optional = TRUE;
    meshDependencies.push_back(meshDependency);
    BObolLodResult stagedResult =
	bobol_lod_directory_result(meshLodRequest, meshDependencies);
    if (!mesh->applyStagedLodResult(stagedResult, &meshLodRequest) ||
	!mesh->lodStagedAvailable.getValue() ||
	mesh->lodResultKind.getValue() != BOBOL_LOD_RESULT_DIRECTORY ||
    ...
    std::vector<BObolLodAttribute> meshAttributes;
    BObolLodAttribute meshAttribute;
    meshAttribute.name = "display.color";
    meshAttribute.value = "255 0 0";
    meshAttributes.push_back(meshAttribute);
    stagedResult = bobol_lod_attributes_result(meshLodRequest,
		   meshAttributes);
    if (!mesh->applyStagedLodResult(stagedResult, &meshLodRequest) ||
    ...
    BObolLodProxy meshProxy;
    meshProxy.kind = BOBOL_LOD_PROXY_OBB;
    ...
    stagedResult = bobol_lod_proxy_result(meshLodRequest, meshProxy, NULL);
    if (!mesh->applyStagedLodResult(stagedResult, &meshLodRequest) ||
	mesh->lodProxyKind.getValue() != BOBOL_LOD_PROXY_OBB ||
	...
	mesh->lodProxyKind.getValue() != BOBOL_LOD_PROXY_NONE ||
	mesh->lodAttributeName.getNum() != 0)
    ...
    if (!bobol_grid_configure_from_view(grid, &gridState, gridModel2View,
					  20.0, 1.0, 400, 200))
    ...
    if (!bobol_grid_configure_from_view(squareGrid, &gridState,
    ...
    (void)bobol_grid_configure_from_view(grid, &gridState,
    ...
    gridState.draw = 0;
    gridState.snap = 1;
    if (!bobol_grid_configure_from_view(grid, &gridState, gridSnapModel2View,
					  20.0, 1.0, 400, 200) ||
    ...
    {
	BObolViewController overlayController(controllerOverlayRoot, NULL);
    ...
    {
	BObolViewController labelController(controllerLabelRoot, NULL);
    ...
    bu_dir(lodCacheDir, MAXPATHLEN, BU_DIR_CURR, "bobol_prototype_lod_cache", NULL);
    ...
    {
	BObolViewController dbController(controllerDbRoot, NULL);
	...
	BObolRtPickCache *rtPickCache = dbController.getRtPickCache(0);
	std::vector<BObolRtPickResult> controllerRtPicks;
	...
	std::vector<BObolRtPickResult> controllerRtMisses;
    ...
    BObolRealizedShapeSummary directMeshSummary;
    ...
    BObolRealizedShapeSummary sphereSummary;
    if (source->getRealizedMeshCount() != 0 ||
    ...
    BObolRealizedShapeSummary coarseSphereSummary;
    ...
    BObolRealizedShapeSummary ellipsoidSummary;
    ...
    BObolRealizedShapeSummary tgcSummary;
    ...
    BObolRealizedShapeSummary torusSummary;
    ...
    BObolRealizedShapeSummary botBelowSummary;
    if (source->getRealizedMeshCount() != 0 ||
    ...
    BObolCompactInstanceHandle botBelowHandle;
    BObolCompactInstanceSummary botBelowInstance;
    ...
    BObolCompactOccurrence botBelowOccurrence;
    ...
    source->setRealizationViewPolicy(TRUE, FALSE, TRUE,
				     ...
				     prototypeViewWidth, prototypeViewHeight,
				     4, 0.0f, 0.0f);
    if (bobol_mesh_lod_cache_update(dbip, "tet.bot") != BRLCAD_OK)
    ...
    BObolCompactInstanceHandle compactHandle0;
    BObolCompactInstanceHandle compactHandle1;
    BObolCompactInstanceSummary compactSummary0;
    BObolCompactInstanceSummary compactSummary1;
    ...
    BObolViewLodState compactViewLodState;
    ...
    BObolLodResult proxyResult =
	bobol_lod_aabb_result(proxyRequest, proxyBounds, NULL);
    ...
    BObolCompactInstanceHandle compactMeshLodHandle;
    BObolCompactInstanceSummary compactMeshLodSummary;
    ...
    meshRequest.qualityTier = BOBOL_LOD_QUALITY_FAST_DISPLAY;
    BObolLodResult meshResult;
    meshResult.request = meshRequest;
    meshResult.cacheKey = bobol_lod_cache_key(meshRequest);
    ...
    BObolRealizedShapeSummary leftMeshSummary;
    BObolRealizedShapeSummary rightMeshSummary;
    if (source->getRealizedMeshCount() != 0 ||
    ...
    BObolCompactInstanceSummary refreshBefore0;
    BObolCompactInstanceSummary refreshBefore1;
    ...
    BObolCompactInstanceSummary refreshAfter0;
    BObolCompactInstanceSummary refreshAfter1;
    ...
    BObolCompactInstanceHandle largeEditHandle;
    BObolCompactInstanceHandle largeOtherHandle;
    BObolCompactInstanceHandle largeMarkerHandle;
    BObolCompactInstanceSummary largeEditBefore;
    BObolCompactInstanceSummary largeOtherBefore;
    BObolCompactInstanceSummary largeMarkerBefore;
    for (int occurrence = 0; occurrence < largeOccurrenceCount; occurrence++) {
	BObolCompactInstanceHandle candidateHandle;
	BObolCompactInstanceSummary candidate;
    ...
    BObolCompactInstanceSummary largeEditState;
    BObolCompactInstanceSummary largeOtherState;
    ...
    BObolCompactInstanceSummary largeEditAfter;
    BObolCompactInstanceSummary largeOtherAfter;
    BObolCompactInstanceSummary largeMarkerAfter;
    if (!largeViews[0]->getCompactInstanceSummary(largeEditHandle,
	largeEditAfter) ||
	!largeViews[0]->getCompactInstanceSummary(largeOtherHandle,
