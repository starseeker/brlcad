...
dm_draw_ticks(struct dm *dmp, struct bv_adc_state *adcp, fastf_t angle)
{
    ...
    /* Tick distance */
    c_tdist = ((fastf_t)(adcp->dv_dist) + RT_VIEW_MAX) * M_SQRT1_2;
...
dm_draw_adc(struct dm *dmp, struct bv_adc_state *adcp, mat_t view2model, mat_t model2view)
{
	...
	bv_adc_model_to_view(adcp, model2view, RT_VIEW_MAX);
	bv_adc_view_to_grid(adcp, model2view);
    } else if (adcp->anchor_pos == 2) {
	bv_adc_grid_to_view(adcp, view2model, RT_VIEW_MAX);
	...
	dx = view_pt[X] * RT_VIEW_MAX - adcp->dv_x;
	dy = view_pt[Y] * RT_VIEW_MAX - adcp->dv_y;

	if (!ZERO(dx) || !ZERO(dy)) {
	    adcp->a1 = RAD2DEG*atan2(dy, dx);
	    adcp->dv_a1 = (1.0 - (adcp->a1 / 45.0)) * RT_VIEW_MAX;
	}
    }
	...
	if (!ZERO(dx) || !ZERO(dy)) {
	    ...
	    adcp->dv_a2 = (1.0 - (adcp->a2 / 45.0)) * RT_VIEW_MAX;
	}
    }
	...
	adcp->dst = dist * RT_INV_VIEW;
	adcp->dv_dist = (dist / M_SQRT1_2) - RT_VIEW_MAX;
    } else
	adcp->dst = (adcp->dv_dist * RT_INV_VIEW + 1.0) * M_SQRT1_2;
    ...
    /* Horizontal */
    dm_draw_line_2d(dmp,
		    GED_TO_PM1(RT_VIEW_MIN), GED_TO_PM1(adcp->dv_y) * dmp->i->dm_aspect,
		    GED_TO_PM1(RT_VIEW_MAX), GED_TO_PM1(adcp->dv_y) * dmp->i->dm_aspect);

    /* Vertical */
    dm_draw_line_2d(dmp,
		    GED_TO_PM1(adcp->dv_x), GED_TO_PM1(RT_VIEW_MAX),
		    GED_TO_PM1(adcp->dv_x), GED_TO_PM1(RT_VIEW_MIN));
