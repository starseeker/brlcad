...
int
rt_pg_plot(struct bu_list *vhead, struct rt_db_internal *ip, const struct bg_tess_tol *UNUSED(ttol), const struct bn_tol *UNUSED(tol), const struct bv_view_info *UNUSED(info))
{
    size_t i;
	...
	pp = &pgp->poly[p];
	RT_ADD_VLIST(vlfree, vhead, &pp->verts[3*(pp->npts-1)],
		     RT_VLIST_LINE_MOVE);
	for (i=0; i < pp->npts; i++) {
	    RT_ADD_VLIST(vlfree, vhead, &pp->verts[3*i],
			 RT_VLIST_LINE_DRAW);
	}
    }
    return 0;		/* OK */
