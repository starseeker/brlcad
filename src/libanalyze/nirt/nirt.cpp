...
_nirt_if_hit(struct application *ap, struct partition *part_head, struct seg *UNUSED(finished_segs))
{
	...
	if (part_nm > 1) {
		...
		/* vlist segment for gap */
		{
		    ...
		    vhead = bg_vlblock_find(nss->i->segs, rgb[RED], rgb[GRN], rgb[BLU]);
		    BG_ADD_VLIST(nss->i->segs->free_vlist_hd, vhead, s->gap_in, BG_VLIST_LINE_MOVE);
		    BG_ADD_VLIST(nss->i->segs->free_vlist_hd, vhead, s->in, BG_VLIST_LINE_DRAW);
		}
		nss->i->b_segs = true;
		s->type = NIRT_PARTITION_SEG;
	    }
	...
	if (!seg_ovlps.size()) {
	    ...
	    vhead = bg_vlblock_find(nss->i->segs, seg_rgb[RED], seg_rgb[GRN], seg_rgb[BLU]);
	    BG_ADD_VLIST(nss->i->segs->free_vlist_hd, vhead, s->in, BG_VLIST_LINE_MOVE);
	    BG_ADD_VLIST(nss->i->segs->free_vlist_hd, vhead, s->out, BG_VLIST_LINE_DRAW);
	} else {
		...
		if (_nirt_in_ovlp(&op, seg_ovlps, curr_dist)) {
		    ...
		    vhead = bg_vlblock_find(nss->i->segs, ovlp_rgb[RED], ovlp_rgb[GRN], ovlp_rgb[BLU]);
		    BG_ADD_VLIST(nss->i->segs->free_vlist_hd, vhead, op->in_point, BG_VLIST_LINE_MOVE);
		    BG_ADD_VLIST(nss->i->segs->free_vlist_hd, vhead, curr_pnt, BG_VLIST_LINE_DRAW);
		    seg_ovlps.erase(op);
		} else {
		    ...
		    // segment.
		    vhead = bg_vlblock_find(nss->i->segs, seg_rgb[RED], seg_rgb[GRN], seg_rgb[BLU]);
		    BG_ADD_VLIST(nss->i->segs->free_vlist_hd, vhead, curr_pnt, BG_VLIST_LINE_MOVE);
		    if (op) {
			BG_ADD_VLIST(nss->i->segs->free_vlist_hd, vhead, op->in_point, BG_VLIST_LINE_DRAW);
...
nirt_init(struct nirt_state *ns)
{
    ...
    n->segs = bg_vlblock_init(&(n->s_vlist), 32);
    n->plot_overlaps = 1;
    n->ret = 0;
...
nirt_destroy(struct nirt_state *ns)
{
    ...
    bg_vlist_cleanup(&(ns->i->s_vlist));
    bg_vlblock_free(ns->i->segs);
