...
{
    bu_setprogname("test_bobol_nanort_render");
    const int width = 180;
    ...
    bobol_init(&nanortManager);
    if (SoDB::getContextManager() != &nanortManager)
	FAIL("bobol_init should install the application-provided NanoRT context manager");
    ...
    BOBOLRenderContextManager osmesaManager;
