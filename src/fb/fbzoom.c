...
/* Pan limits */
#define MaxXPan ((int)imgstream_fb_width(fbp)-1)
#define MaxYPan ((int)imgstream_fb_height(fbp)-1)
#define MinPan (0)
...
main(int argc, char **argv)
{
    ...
    if (remaining == 4) {
	imgstream_fb_view(fbp, xPan, yPan, xZoom, yZoom);
    }

    imgstream_fb_getview(fbp, &xPan, &yPan, &xZoom, &yZoom);
    ...
    PanFactor = (int)imgstream_fb_width(fbp)/16;
    V_MAX(PanFactor, 2);
	...
	if (new_xPan != xPan || new_yPan != yPan
	    ...
	    if (imgstream_fb_view(fbp, new_xPan, new_yPan,
    ...
    reset_Tty(0);
    (void)imgstream_fb_view(fbp, xPan, yPan, xZoom, yZoom);
...
doKeyPad(void)
{
	...
	case 'C' :
	    new_xPan = (int)imgstream_fb_width(fbp)/2;
	    new_yPan = (int)imgstream_fb_height(fbp)/2;
	    break;
	case 'z' :				/* Reset Zoom */
	case 'Z' :
	    new_xZoom = 1;
