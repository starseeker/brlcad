...
nmg_vu_to_vlist(struct bu_list *vhead, const struct vertexuse *vu, struct bu_list *vlfree)
{
	...
	NMG_CK_VERTEX_G(vg);
	BG_ADD_VLIST(vlfree, vhead, vg->coord, BG_VLIST_LINE_MOVE);
	BG_ADD_VLIST(vlfree, vhead, vg->coord, BG_VLIST_LINE_DRAW);
    }
}
...
nmg_eu_to_vlist(struct bu_list *vhead, const struct bu_list *eu_hd, struct bu_list *vlfree)
{
	...
	BG_ADD_VLIST(vlfree, vhead, vg->coord, BG_VLIST_LINE_MOVE);
	BG_ADD_VLIST(vlfree, vhead, vgmate->coord, BG_VLIST_LINE_DRAW);
    }
}


/**
 * Plot a single loopuse into a bg_vlist chain headed by vhead.
 *
...

{
	...
	if (isfirst) {
		...
		BG_ADD_VLIST(vlfree, vhead, normal, BG_VLIST_POLY_START);
		if (poly_markers & NMG_VLIST_STYLE_USE_VU_NORMALS
		    && vu->a.magic_p) {
		    BG_ADD_VLIST(vlfree, vhead,
				 vu->a.plane_p->N,
				 BG_VLIST_POLY_VERTNORM);
		}
		BG_ADD_VLIST(vlfree, vhead, vg->coord, BG_VLIST_POLY_MOVE);
	    } else {
		/* move */
		BG_ADD_VLIST(vlfree, vhead, vg->coord, BG_VLIST_LINE_MOVE);
	    }
		...
		BG_ADD_VLIST(vlfree, vhead, vg->coord, BG_VLIST_POLY_DRAW);
	    } else {
		/* Draw */
		BG_ADD_VLIST(vlfree, vhead, vg->coord, BG_VLIST_LINE_DRAW);
	    }
	...
	nmg_cnurb_to_vlist(vhead, eu, 10,
			   ...
			   BG_VLIST_POLY_DRAW : BG_VLIST_LINE_DRAW, vlfree);
    }
	...
	if (poly_markers & NMG_VLIST_STYLE_POLYGON) {
	    ...
	    BG_ADD_VLIST(vlfree, vhead, first_vg->coord, BG_VLIST_POLY_END);
	} else {
	    /* Draw */
	    BG_ADD_VLIST(vlfree, vhead, first_vg->coord, BG_VLIST_LINE_DRAW);
	}
    }
	...
	if (fp) {
		...
		BG_ADD_VLIST(vlfree, vhead, centroid, BG_VLIST_LINE_MOVE);
		VJOIN1(tip, centroid, f, normal);
		BG_ADD_VLIST(vlfree, vhead, tip, BG_VLIST_LINE_DRAW);
	    }
	}
...
nmg_snurb_fu_to_vlist(struct bu_list *vhead, const struct faceuse *fu, int poly_markers, struct bu_list *vlfree)
{
	...
	BG_ADD_VLIST(vlfree, vhead, mid_srf, BG_VLIST_LINE_MOVE);
...
{
    struct bg_vlblock *vbp = bg_vlblock_init(vlfree, 32);
...
void
nmg_vlblock_v(struct bg_vlblock *vbp, const struct vertex *v, long *tab, struct bu_list *vlfree)
{
    pointp_t p;
    struct bu_list *vh;
    ...
    vh = bg_vlblock_find(vbp, 255, 255, 255);
    BG_ADD_VLIST(vlfree, vh, p, BG_VLIST_LINE_MOVE);
    BG_ADD_VLIST(vlfree, vh, p, BG_VLIST_LINE_DRAW);
}


void
nmg_vlblock_e(struct bg_vlblock *vbp, const struct edge *e, long *tab, int red, int green, int blue, struct bu_list *vlfree)
{
    pointp_t p0, p1;
    point_t end0, end1;
    vect_t v;
    ...
    vh = bg_vlblock_find(vbp, red, green, blue);
    BG_ADD_VLIST(vlfree, vh, end0, BG_VLIST_LINE_MOVE);
    BG_ADD_VLIST(vlfree, vh, end1, BG_VLIST_LINE_DRAW);
...
void
nmg_vlblock_eu(struct bg_vlblock *vbp, const struct edgeuse *eu, long *tab, int red, int green, int blue, int fancy, struct bu_list *vlfree)
{
	...
	*eu->up.lu_p->up.magic_p == NMG_FACEUSE_MAGIC) {
	 ...
	 */
	vh = bg_vlblock_find(vbp, red, green, blue);
	BG_ADD_VLIST(vlfree, vh, base, BG_VLIST_LINE_MOVE);
	BG_ADD_VLIST(vlfree, vh, tip, BG_VLIST_LINE_DRAW);
	 ...
	 */
	nmg_eu_radial(eu, radial_tip);
	vh = bg_vlblock_find(vbp, red, green-20, blue);
	BG_ADD_VLIST(vlfree, vh, tip, BG_VLIST_LINE_MOVE);
	BG_ADD_VLIST(vlfree, vh, radial_tip, BG_VLIST_LINE_DRAW);
	...
	vh = bg_vlblock_find(vbp, red, green, blue);
	BG_ADD_VLIST(vlfree, vh, tip, BG_VLIST_LINE_MOVE);
	BG_ADD_VLIST(vlfree, vh, next_base, BG_VLIST_LINE_DRAW);
    }
}
...
nmg_vlblock_euleft(struct bu_list *vh, const struct edgeuse *eu, const point_t center, const mat_t mat, const vect_t xvec, const vect_t yvec, double len, struct bu_list *vlfree, const struct bn_tol *tol)
{
    ...
    BG_ADD_VLIST(vlfree, vh, center, BG_VLIST_LINE_MOVE);
    BG_ADD_VLIST(vlfree, vh, tip, BG_VLIST_LINE_DRAW);
    ...
    char_scale = len * 0.05;
    bg_vlist_3string(vh, vlfree, str, tip, mat, char_scale);
}
 ...
 */
void
nmg_vlblock_around_eu(struct bg_vlblock *vbp, const struct edgeuse *arg_eu, long *tab, int fancy, struct bu_list *vlfree, const struct bn_tol *tol)
{
	...
	/* Yellow, for now */
	vh = bg_vlblock_find(vbp, 255, 200, 0);
    } else {
...
void
nmg_vlblock_lu(struct bg_vlblock *vbp, const struct loopuse *lu, long *tab, int red, int green, int blue, int fancy, struct bu_list *vlfree)
{
    struct edgeuse *eu;
    uint32_t magic1;
    struct vertexuse *vu;
...
void
nmg_vlblock_fu(struct bg_vlblock *vbp, const struct faceuse *fu, long *tab, int fancy, struct bu_list *vlfree)
{
    struct loopuse *lu;
...
void
nmg_vlblock_s(struct bg_vlblock *vbp, const struct shell *s, int fancy, struct bu_list *vlfree)
{
    struct faceuse *fu;
    struct loopuse *lu;
    struct edgeuse *eu;
...
void
nmg_vlblock_r(struct bg_vlblock *vbp, const struct nmgregion *r, int fancy, struct bu_list *vlfree)
{
    struct shell *s;
...
void
nmg_vlblock_m(struct bg_vlblock *vbp, const struct model *m, int fancy, struct bu_list *vlfree)
{
    struct nmgregion *r;
...
static void
nmg_line_layer_add(struct bg_line_layer_builder *plot, int red, int green, int blue, const point_t p, int cmd)
{
    if (!plot || !p)
	return;
    (void)bg_line_layer_builder_add(plot, red, green, blue, p, cmd);
}

static void
nmg_line_layer_v(struct bg_line_layer_builder *plot, const struct vertex *v, long *tab)
{
    pointp_t p;
    ...
    nmg_line_layer_add(plot, 255, 255, 255, p, BG_GEOMETRY_LINE_MOVE);
    nmg_line_layer_add(plot, 255, 255, 255, p, BG_GEOMETRY_LINE_DRAW);
}

static void
nmg_line_layer_e(struct bg_line_layer_builder *plot, const struct edge *e, long *tab, int red, int green, int blue)
{
    pointp_t p0, p1;
    point_t end0, end1;
    vect_t v;
    ...
    nmg_line_layer_add(plot, red, green, blue, end0, BG_GEOMETRY_LINE_MOVE);
    nmg_line_layer_add(plot, red, green, blue, end1, BG_GEOMETRY_LINE_DRAW);

    nmg_line_layer_v(plot, e->eu_p->vu_p->v_p, tab);
    nmg_line_layer_v(plot, e->eu_p->eumate_p->vu_p->v_p, tab);
}

static void
nmg_line_layer_eu(struct bg_line_layer_builder *plot, const struct edgeuse *eu, long *tab, int red, int green, int blue, int fancy)
{
    point_t base = VINIT_ZERO;
    point_t next_base = VINIT_ZERO;
    point_t radial_tip = VINIT_ZERO;
    ...
    nmg_line_layer_e(plot, eu->e_p, tab, red, green, blue);
	...
	*eu->up.lu_p->up.magic_p == NMG_FACEUSE_MAGIC) {
	...
	nmg_line_layer_add(plot, red, green, blue, base, BG_GEOMETRY_LINE_MOVE);
	nmg_line_layer_add(plot, red, green, blue, tip, BG_GEOMETRY_LINE_DRAW);

	nmg_eu_radial(eu, radial_tip);
	nmg_line_layer_add(plot, red, green - 20, blue, tip, BG_GEOMETRY_LINE_MOVE);
	nmg_line_layer_add(plot, red, green - 20, blue, radial_tip, BG_GEOMETRY_LINE_DRAW);

	nmg_eu_next_base(eu, next_base);
	red *= 0.5;
	green *= 0.5;
	blue *= 0.5;
	nmg_line_layer_add(plot, red, green, blue, tip, BG_GEOMETRY_LINE_MOVE);
	nmg_line_layer_add(plot, red, green, blue, next_base, BG_GEOMETRY_LINE_DRAW);
    }
}

static void
nmg_line_layer_euleft(struct bg_line_layer_builder *plot, const struct edgeuse *eu, const point_t center, int red, int green, int blue, double len, const struct bn_tol *tol)
{
    vect_t left;
    point_t tip;
    fastf_t fan_len;
    ...
    nmg_line_layer_add(plot, red, green, blue, center, BG_GEOMETRY_LINE_MOVE);
    nmg_line_layer_add(plot, red, green, blue, tip, BG_GEOMETRY_LINE_DRAW);
}

void
nmg_line_layer_around_eu(struct bg_line_layer_builder *plot, const struct edgeuse *arg_eu, long *tab, int fancy, const struct bn_tol *tol)
{
    const struct edgeuse *orig_eu;
    const struct edgeuse *eu;
    vect_t xvec, yvec, zvec;
    ...
    do {
	if (fancy)
	    nmg_line_layer_euleft(plot, eu, center, 255, 200, 0, len, tol);

	nmg_line_layer_eu(plot, eu, tab, 80, 100, 170, 3);
	eu = eu->eumate_p;
...
static void
nmg_line_layer_lu(struct bg_line_layer_builder *plot, const struct loopuse *lu, long *tab, int red, int green, int blue, int fancy)
{
    struct edgeuse *eu;
    uint32_t magic1;
    struct vertexuse *vu;
	...
	vu = BU_LIST_PNEXT(vertexuse, &lu->down_hd);
	NMG_CK_VERTEXUSE(vu);
	nmg_line_layer_v(plot, vu->v_p, tab);
    } else if (magic1 == NMG_EDGEUSE_MAGIC) {
	for (BU_LIST_FOR(eu, edgeuse, &lu->down_hd)) {
	    nmg_line_layer_eu(plot, eu, tab, red, green, blue, fancy);
	}
    }
}

static void
nmg_line_layer_fu(struct bg_line_layer_builder *plot, const struct faceuse *fu, long *tab, int fancy)
{
    struct loopuse *lu;
    ...
    for (BU_LIST_FOR(lu, loopuse, &fu->lu_hd)) {
	nmg_line_layer_lu(plot, lu, tab, 80, 100, 170, fancy ? fancy : 0);
    }
}

void
nmg_line_layer_s(struct bg_line_layer_builder *plot, const struct shell *s, int fancy)
{
    struct faceuse *fu;
    struct loopuse *lu;
    struct edgeuse *eu;
    ...
    tab = (long *)bu_calloc(m->maxindex + 1, sizeof(long), "nmg line layer tab[]");

    for (BU_LIST_FOR(fu, faceuse, &s->fu_hd)) {
	NMG_CK_FACEUSE(fu);
	nmg_line_layer_fu(plot, fu, tab, fancy);
    }

    for (BU_LIST_FOR(lu, loopuse, &s->lu_hd)) {
	NMG_CK_LOOPUSE(lu);
	if (fancy) {
	    nmg_line_layer_lu(plot, lu, tab, 255, 0, 0, fancy);
	} else {
	    nmg_line_layer_lu(plot, lu, tab, 200, 0, 0, 0);
	}
    }
	...
	if (fancy) {
	    nmg_line_layer_eu(plot, eu, tab, 200, 200, 0, fancy);
	} else {
	    nmg_line_layer_eu(plot, eu, tab, 200, 200, 0, 0);
	}
    }
    if (s->vu_p) {
	nmg_line_layer_v(plot, s->vu_p->v_p, tab);
    }

    bu_free((char *)tab, "nmg line layer tab[]");
}

void
nmg_line_layer_r(struct bg_line_layer_builder *plot, const struct nmgregion *r, int fancy)
{
    struct shell *s;
    ...
    for (BU_LIST_FOR(s, shell, &r->s_hd)) {
	nmg_line_layer_s(plot, s, fancy);
    }
}

void
nmg_line_layer_m(struct bg_line_layer_builder *plot, const struct model *m, int fancy)
{
    struct nmgregion *r;
 ...
 */
void
nmg_pl_edges_in_2_shells(struct bg_vlblock *vbp, long *b, const struct edgeuse *eu, int fancy, struct bu_list *vlfree, const struct bn_tol *tol)
{
...
static void
show_broken_vu(struct bg_vlblock *vbp, const struct vertexuse *vu, struct bu_list *vlfree)
{
    pointp_t p;
    struct bu_list *vh;
    struct vertex *v;
    ...
    VMOVE(pt, p);
    pt[0] += 0.05;
    BG_ADD_VLIST(vlfree, vh, pt, BG_VLIST_LINE_MOVE);
    VMOVE(pt, p);
    pt[0] -= 0.05;
    BG_ADD_VLIST(vlfree, vh, pt, BG_VLIST_LINE_DRAW);
...
static void
show_broken_e(struct bg_vlblock *vbp, const struct edgeuse *eu, struct bu_list *vlfree)
{
    pointp_t p0, p1;
    point_t end0, end1;
    vect_t v;
...
static void
show_broken_eu(struct bg_vlblock *vbp, const struct edgeuse *eu, int fancy, struct bu_list *vlfree)
{
    struct bu_list *vh;
    int red, green, blue;
    point_t base = VINIT_ZERO;
	...
	*eu->up.lu_p->up.magic_p == NMG_FACEUSE_MAGIC) {
	...
	nmg_eu_next_base(eu, next_base);
	vh = bg_vlblock_find(vbp, 0, 100, 0);
	BG_ADD_VLIST(vlfree, vh, tip, BG_VLIST_LINE_MOVE);
	BG_ADD_VLIST(vlfree, vh, next_base, BG_VLIST_LINE_DRAW);
    }
...
static void
show_broken_lu(struct bg_vlblock *vbp, const struct loopuse *lu, int fancy, struct bu_list *vlfree)
{
...
static void
show_broken_fu(struct bg_vlblock *vbp, const struct faceuse *fu, int fancy, struct bu_list *vlfree)
{
    register struct loopuse *lu;
...
static void
show_broken_s(struct bg_vlblock *vbp, const struct shell *s, int fancy, struct bu_list *vlfree)
{
    struct faceuse *fu;
    struct loopuse *lu;
    struct edgeuse *eu;
...
static void
show_broken_r(struct bg_vlblock *vbp, const struct nmgregion *r, int fancy, struct bu_list *vlfree)
{
    register struct shell *s;
...
static void
show_broken_m(struct bg_vlblock *vbp, const struct model *m, int fancy, struct bu_list *vlfree)
{
    register struct nmgregion *r;
...
{
    static struct bg_vlblock *vbp = (struct bg_vlblock *)NULL;
    struct model *m;
...
/* printf("showing broken stuff\n"); */
     ...
     * "copy" flag on nmg_plot_anim_upcall() means that the vlist
...
/** nmg_hack_snurb(&n, fg);	/ XXX */
	...
	BG_ADD_VLIST(vlfree, vhead, vp, BG_VLIST_LINE_MOVE);
	vp += coords;
	for (j = 1; j < c->s_size[1]; j++) {
	    BG_ADD_VLIST(vlfree, vhead, vp, BG_VLIST_LINE_DRAW);
	    vp += coords;
	}
    }
...
/* BG_VLIST_LINE_DRAW, etc. */
{
	...
	for (i = 1; i < c->c_size-1; i++) {
	    BG_ADD_VLIST(vlfree, vhead, vp, cmd);
	    vp += coords;
	}
    } else {
	...
	for (i = 0; i < n_interior; i++) {
	    ...
	    BG_ADD_VLIST(vlfree, vhead, final, cmd);
	    vp += coords;
	}
    }
