...
/** @addtogroup libBObol */
/** @{ */
/** @file libBObol/evaluated_wire.cpp
 *
 * libBObol compatibility adapter for librt evaluated-wireframe output.
 *
 */
/** @} */
...
static int
bobol_vlist_line_count(const struct bu_list *vhead, size_t *count_out)
{
    rt_vlist *vp = NULL;
...
static int
bobol_vlist_export_line_set(const struct bu_list *vhead,
			      point_t **points_out,
			      int **commands_out,
			      size_t *count_out)
{
    ...
    *count_out = 0;
    ...
    if (bobol_vlist_line_count(vhead, &count) != BRLCAD_OK)
	return BRLCAD_ERROR;
    if (!count)
	return BRLCAD_OK;
    ...
    size_t *count_out)
{
	...
	*count_out = 0;
    ...
    if (ret == BRLCAD_OK)
	ret = bobol_vlist_export_line_set(&vhead, points_out, commands_out,
					    count_out);
...
void
bobol_evaluated_wire_line_set_free(point_t *points, int *commands)
{
    if (points)
