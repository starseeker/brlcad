...
static void
mged_view_ring_save_current(struct mged_state *s, struct view_ring *vrp)
{
    if (!vrp)
	return;

    ged_view_context_rotation_get(vrp->vr_rot_mat, view_state->vs_gvp);
    ged_view_context_center_get(vrp->vr_tvc_mat, view_state->vs_gvp);
    vrp->vr_scale = ged_view_context_scale_get(view_state->vs_gvp);
}

static void
mged_view_ring_restore_current(struct mged_state *s, struct view_ring *vrp)
{
    point_t view_center;
    ...
    ged_view_context_rotation_set(view_state->vs_gvp, vrp->vr_rot_mat);
    MAT_DELTAS_GET_NEG(view_center, vrp->vr_tvc_mat);
    ged_view_context_center_vec_set(view_state->vs_gvp, view_center);
    ged_view_context_scale_set(view_state->vs_gvp, vrp->vr_scale);
}
...
static int
mged_knob_edit_process(struct mged_state *s,
                       const struct knob_token_entry *ke,
                       fastf_t fval,
                       int incr_flag,
                       char origin,
                       vect_t rvec, int *did_rot,
                       vect_t tvec, int *did_tran,
                       int *did_sca)
{
    if (!s || !ke) return BRLCAD_ERROR;

    switch (ke->type) {
        case KNOB_ROT_RATE: {
            int axis = ke->axis;
            if (axis < 0) return BRLCAD_ERROR;
        ...
        case KNOB_TRA_ABS: {
            int axis = ke->axis;
            fastf_t view_scale = ged_view_context_scale_get(view_state->vs_gvp);
            if (axis < 0) return BRLCAD_ERROR;
            fastf_t *abs_arr = NULL, *last_arr = NULL;
            switch (mged_variables->mv_coords) {
                case 'm':
            ...
            fastf_t sf = fval * s->dbip->dbi_local2base / view_scale;
            fastf_t delta;
            if (incr_flag) {
                abs_arr[axis] += sf;
                last_arr[axis] = abs_arr[axis];
                ...
                delta = fval - last_arr[axis] * view_scale * s->dbip->dbi_base2local;
                abs_arr[axis] = sf;
                last_arr[axis] = abs_arr[axis];
            }
            tvec[axis] += delta;
        ...
        case KNOB_SCA_RATE: {
            if (incr_flag) MEDIT(s)->k.sca += fval;
            else MEDIT(s)->k.sca = fval;
            return BRLCAD_OK;
        }
        case KNOB_SCA_ABS: {
            if (incr_flag) MEDIT(s)->k.sca_abs += fval;
            else MEDIT(s)->k.sca_abs = fval;
            *did_sca = 1;
            return BRLCAD_OK;
        }
        default:
	  ...
	  mat_t newrot)
{
	...
	case 'o':
	    ...
	    break;
	case 'v':
	    ged_view_context_rotation_get(view_rotation, view_state->vs_gvp);
	    bn_mat_inv(temp1, view_rotation);

	    /* transform into model rotations */
	    bn_mat_mul(temp2, temp1, newrot);
	    bn_mat_mul(newrot, temp2, view_rotation);
	    break;
    }
	   ...
	   const vect_t tvec)
{
	...
	case 'v':
	default:
	    view_scale = ged_view_context_scale_get(view_state->vs_gvp);
	    ged_view_context_view2model_get(view2model, view_state->vs_gvp);
	    ged_view_context_center_get(view_center, view_state->vs_gvp);

	    VSCALE(p2, tvec, s->dbip->dbi_local2base / view_scale);
	    MAT4X3PNT(work, view2model, p2);
...
/* This is the function to replace with librt based logic
 * when the time comes */
static int
mged_knob_edit_apply(struct mged_state *s,
                     char origin,
                     const vect_t rvec, int did_rot,
                     const vect_t tvec, int did_tran,
                     int did_sca)
{
    ...
    if (did_rot) {
        /* Reuse existing mged_erot_xyz path */
...
cmd_size(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
    ...
    if (ret == BRLCAD_OK) {
	fastf_t view_scale = ged_view_context_scale_get(view_state->vs_gvp);
	fastf_t view_initial_scale = ged_view_context_initial_scale_get(view_state->vs_gvp);
	fastf_t view_absolute_scale = 1.0 - view_scale / view_initial_scale;

	if (view_absolute_scale < 0.0) {
	    view_absolute_scale /= 9.0;
	}
	ged_view_context_absolute_scale_set(view_state->vs_gvp, view_absolute_scale);
...
size_reset(struct mged_state *s)
{
    ...
    ged_view_context_initial_scale_set(view_state->vs_gvp,
	    ged_view_context_scale_get(view_state->vs_gvp));
    mged_refresh_request_view(s, view_state, GED_VIEW_REFRESH_VIEW);
}
...
f_status(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
	...
	bu_vls_printf(&vls, "Viewscale=%f (%f mm)\n",
		      view_scale * s->dbip->dbi_base2local, view_scale);
	bu_vls_printf(&vls, "s->dbip->dbi_base2local=%f\n", s->dbip->dbi_base2local);
	Tcl_AppendResult(interp, bu_vls_addr(&vls), (char *)NULL);
	bu_vls_free(&vls);

	mged_bn_mat_print(interp, "toViewcenter", view_center);
	mged_bn_mat_print(interp, "Viewrot", view_rotation);
	mged_bn_mat_print(interp, "model2view", model2view);
	mged_bn_mat_print(interp, "view2model", view2model);
	...
	fastf_t view_scale = ged_view_context_scale_get(view_state->vs_gvp);
	bu_vls_printf(&vls, "%f", view_scale * s->dbip->dbi_base2local);
	Tcl_AppendResult(interp, bu_vls_addr(&vls), (char *)NULL);
	bu_vls_free(&vls);
	return TCL_OK;
    }
...
_fill_shape_cb(const struct ged_draw_shape_record *rec, void *ud)
{
    ...
    if (d->exact && d->nm_pieces != rec->fullpath->fp_len) return 1;
    /* XXX Could this make use of db_full_path_subset()? */
    if (d->nmatch == 0 || d->nmatch != d->ri) {
	fi = (int)rec->fullpath->fp_len - 1;
	if (DB_FULL_PATH_GET(rec->fullpath, fi) == d->dp) {
	    a_new_match = 1;
	    fj = (int)d->nm_pieces - 1;
	    for (; a_new_match && (fi >= 0) && (fj >= 0); --fi, --fj) {
		fsname = DB_FULL_PATH_GET(rec->fullpath, fi)->d_namep;
...
static void
update_view_knob_rate_flags(struct rt_view_knobs *k)
{
    if (!k) return;
...
static void
update_edit_knob_rate_flags(struct rt_edit_knobs *k)
{
    if (!k) return;
    k->rot_m_flag = (!ZERO(k->rot_m[X]) || !ZERO(k->rot_m[Y]) || !ZERO(k->rot_m[Z]));
    k->rot_o_flag = (!ZERO(k->rot_o[X]) || !ZERO(k->rot_o[Y]) || !ZERO(k->rot_o[Z]));
...
mged_print_knobvals(struct mged_state *s, Tcl_Interp *interp)
{
	...
	if (s->s_edit->es_edclass == EDIT_CLASS_SCALE && mged_variables->mv_transform == 'e') {
	    ...
	    bu_vls_printf(&vls, "aS = %f\n", ged_view_context_absolute_scale_get(view_state->vs_gvp));
	}
		...
		const char *token)
{
	...
	ged_view_context_knobs_reset(view_state->vs_gvp, RT_VIEW_KNOBS_ALL);
	if (MEDIT(s)) {
	    rt_edit_knobs_reset(&MEDIT(s)->k, RT_EDIT_KNOBS_RATE);
	}
...
f_knob(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
	...
	if (token_is_core_view(ke)) {
		...
		if (using_librt_edit) {
		    ...
		    if (!re)
			goto usage;
		    void *v = view_state->vs_gvp;
		    char save_coord = ged_view_context_coord_get(v);
		    ged_view_context_coord_set(v, mged_variables->mv_coords);
		    if (rt_edit_knob_cmd_process_bsg(re,
				&edit_rvec, &edit_do_rot,
				&edit_tvec, &edit_do_tran,
				&edit_do_sca,
				v, token, fval,
				origin, incr_flag, NULL) != BRLCAD_OK) {
			ged_view_context_coord_set(v, save_coord);
			goto usage;
		    }
		    ged_view_context_coord_set(v, save_coord);
		} else {
		...
		if (s->dbip) {
		    ged_view_context_unit_conversion_set(view_state->vs_gvp,
			    s->dbip->dbi_local2base,
			    s->dbip->dbi_base2local);
		}
		int model_mode = model_flag || (mged_variables->mv_coords == 'm' && !view_flag);
		if (ged_view_context_knobs_cmd_process(&view_rvec, &view_do_rot, &view_tvec, &view_do_tran,
			    view_state->vs_gvp,
			    token, fval,
	...
	if (view_do_tran) {
	    ged_view_context_knobs_translate(view_state->vs_gvp, view_tvec, model_mode_final);
	}

	if (view_do_rot) {
	    ged_view_context_knobs_rotate(view_state->vs_gvp, view_rvec, origin, vcoords,
    ...
    /* Sync view_state->k with current retained view knobs before rate flag calc */
    ...
    /* Synchronize MGED's authoritative knob state into the active retained view so that
     ...
     * would drop the earlier MGED changes because the retained view knob
     * record lags behind view_state->k. */
    if (view_state && view_state->vs_gvp)
	ged_view_context_knobs_state_set(view_state->vs_gvp, &view_state->k);
...
mged_zoom(struct mged_state *s, double val)
{
    ...
    if (active_view &&
	ged_view_context_lod_policy_get(&lod_policy, active_view) &&
...
f_slewview(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
    ...
    ged_view_context_center_get(view_center, view_state->vs_gvp);
    MAT_DELTAS_GET_NEG(new_model_center, view_center);
...
mged_svbase(struct mged_state *s)
{
    ...
    ged_view_context_initial_scale_set(view_state->vs_gvp, view_scale);
	...
	ged_view_context_knobs_state_set(view_state->vs_gvp, &view_state->k);
	ged_view_context_knobs_reset(view_state->vs_gvp, RT_VIEW_KNOBS_ABS);
	ged_view_context_knobs_state_get(&view_state->k, view_state->vs_gvp);
    }
    ...
    ged_view_context_absolute_scale_set(view_state->vs_gvp, 0.0);

    /* Sync active retained view knob state */
    if (view_state->vs_gvp) {
	ged_view_context_knobs_state_set(view_state->vs_gvp, &view_state->k);
    }
...
f_view_ring(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
	...
	mged_view_ring_save_current(s, view_state->vs_current_view);
	...
	mged_view_ring_restore_current(s, view_state->vs_current_view);
...
cmd_mrot(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
	...
	return mged_erot(s, ged_view_context_coord_get(view_state->vs_gvp),
		ged_view_context_rotate_about_get(view_state->vs_gvp), rmat);
    } else {
	int ret;
...
cmd_rot(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
	...
	return mged_erot(s, coord, ged_view_context_rotate_about_get(view_state->vs_gvp),
		rmat);
    } else {
	int ret;
...
mged_vscale(struct mged_state *s, fastf_t sfactor)
{
    ...
    view_scale = ged_view_context_scale_get(view_state->vs_gvp) * sfactor;

    if (view_scale < RT_VIEW_MIN_SIZE) {
	view_scale = RT_VIEW_MIN_SIZE;
    }

    ged_view_context_scale_set(view_state->vs_gvp, view_scale);
    f = view_scale / ged_view_context_initial_scale_get(view_state->vs_gvp);

    if (f >= 1.0) {
	ged_view_context_absolute_scale_set(view_state->vs_gvp, (f - 1.0) / -9.0);
    } else {
	ged_view_context_absolute_scale_set(view_state->vs_gvp, 1.0 - f);
    }
...
cmd_sca(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
	...
	f = ged_view_context_scale_get(view_state->vs_gvp) /
	    ged_view_context_initial_scale_get(view_state->vs_gvp);
