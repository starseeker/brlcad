...
static SbBool
snap_source_full_detail_result_valid(const BObolSourceMeshRequest &sourceRequest,
...
static int
snap_source_mesh_face_index(const BObolLodMeshPayload &mesh,
...
static int
snap_source_mesh_vertex_index(const BObolLodMeshPayload &mesh,
			      int localIndex)
{
    if (localIndex < 0)
	return localIndex;
