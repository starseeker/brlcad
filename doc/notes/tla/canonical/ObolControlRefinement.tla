...
VARIABLES facts, constraint, error, probeFact

vars == <<facts, constraint, error, probeFact>>

\* Constraint and error are typed terminal evidence, not work owners.  They
\* refine the production outcome fields independently of the obligation mask.
Foreground == Obligations(facts) \ Background
Terminal == Foreground = {}
Ready == Terminal /\ ~constraint /\ ~error
Constrained == Terminal /\ constraint
Failed == Terminal /\ error

TypeOK ==
    /\ facts \subseteq RepresentativeFacts
    /\ constraint \in BOOLEAN
    /\ error \in BOOLEAN
    /\ ~(constraint /\ error)
    /\ probeFact \in Facts
...
ReadyIsValid == Ready => Terminal /\ ~constraint /\ ~error
OutcomeEvidenceIsExclusive == ~(constraint /\ error)
TerminalHasTypedOutcome == Terminal => Ready \/ Constrained \/ Failed
...
DischargeOwner ==
    ...
    /\ UNCHANGED <<constraint, error, probeFact>>
