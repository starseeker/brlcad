...
static void
_view_vZ_export_record(const struct ged_draw_view_db_object_record *rec, struct _view_vZ_ctx *ctx)
{
    if (!rec || !ctx)
	return;
    ...
    sctx.ctx = ctx;
    sctx.rec = rec;
    if (ged_draw_view_db_object_record_foreach_segment(rec,
	    _view_vZ_segment_cb, &sctx))
	return;
...
static int
_view_vZ_export_record_cb(const struct ged_draw_view_db_object_record *rec, void *data)
{
    _view_vZ_export_record(rec, (struct _view_vZ_ctx *)data);
    return 1;
}

static void
_view_vZ_visit_db_exports(void *view_ctx, struct _view_vZ_ctx *ctx)
{
    ged_draw_foreach_visible_view_db_object_record(view_ctx,
	    _view_vZ_export_record_cb, ctx);
}
...
_view_cmd_independent(void *bs, int argc, const char **argv)
{
    ...
    void *view_ctx = ged_view_find_ctx(gedp, argv[0]);
    if (!view_ctx) {
...
_view_cmd_selections(void *bs, int argc, const char **argv)
{
    ...
    bu_vls_printf(gd->gedp->ged_result_str, "%zu",
		  ged_draw_view_context_selection_count(gd->cv));
...
_view_cmd_vZ(void *bs, int argc, const char **argv)
{
	...
	if (bu_vls_strlen(&calc_target)) {
	    fastf_t vZ = 0.0;
	    if (ged_draw_view_context_feature_depth(gd->cv, bu_vls_cstr(&calc_target),
			calc_mode, &vZ)) {
	    ...
	    ctx.have_vz = 0;
	    ged_draw_view_context_feature_depth_foreach(gd->cv, calc_mode,
