...
png_color_component(float value)
{
    if (value <= 0.0f)
	return 0;
    if (value >= 1.0f)
...
static void
dl_png(struct ged_view_context *view_ctx, mat_t model2view, fastf_t perspective, vect_t eye_pos, size_t size, size_t half_size, unsigned char **image)
{
    mat_t newmat;
    matp_t mat;
