...
ProducerStates == {"building", "complete", "constrained", "cancelled"}
...
\* A constraint belongs to the request which observed it, not to immutable
\* pages already completed by the asset producer.  Once a newer demand has
\* retired that failure, the same producer resumes from its retained prefix.
RestartConstrained ==
    ...
    /\ UNCHANGED <<inputOpen, demand, readyPages, pageDemand,
                    finalGeometry, finalPresentedDemand, coverage,
                    cacheMarked, failureDemand, framePending>>
...
PublishFinal ==
    ...
    /\ UNCHANGED <<inputOpen, demand, producer, readyPages, pageDemand,
                    finalGeometry, cacheMarked, failureDemand>>
...
Quiescent ==
    /\ ~inputOpen
    /\ ~framePending
    /\ producer \in {"complete", "constrained", "cancelled"}
    /\ UNCHANGED vars
...
Terminal ==
    ...
    /\ ~framePending
    /\ producer \in {"complete", "constrained", "cancelled"}
    /\ (finalPresentedDemand = demand \/ failureDemand = demand \/
         producer = "cancelled")
