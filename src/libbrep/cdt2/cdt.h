...
Make much heavier use of the splitting process being used for the "simpler"
refinement - that is checking for longer edges in triangles, that might
actually handle the full "what to refine" problem for the initial breakdown
of the mesh - just ID interior verts and edge segments that are part of
"long" edges of distorted triangles, and split those until triangles
end up well behaved.

Might do the same in the overlap refinement process - if we can't move
verts, there will need to be a radiating pattern of smaller triangles
spreading out from nearby verts, and we currently don't get that.  The
refinement can't be local either, once repair passes are made, since
we're getting crazy triangles connecting "unimpacted" regions to refinement
regions...
...
class mesh_point_t {
    public:
	/////////////////////////////////////////////////////////////////////////////
	// Geometry information - point and associated spatial data
	/////////////////////////////////////////////////////////////////////////////

	// 3D point
	ON_3dPoint p;

	// Unlike the 3D location, the normals associated with a point may vary
	// at edges depending on which BRep face is involved.  For points with
	// type of B_VERT or B_EDGE, need to supply the face_index to look up
	// the correct contextual normal.  (For points of type B_SURF this
	// parameter is unnecessary and ignored.)
	//
	// Note - if calling code is using an ON_BrepTrim pointer or the trim
	// index, use FaceIndexOf() to get the associated face index for a normal
	// lookup:
	//
	// trim->FaceIndexOf();
	// brep->m_T[trim_ind].FaceIndexOf();
	ON_3dVector norm(int f_ind = -1);
	...
	// Bounding box around the point, with sized defined based on connected
	// edge lengths.
	ON_BoundingBox bb;
	...
	// associated information that needs updating.  These are low
	// level operations used when manipulating mesh structure.
	...
	// BRep is connected to this particular point.  Which set of information is
	...
	bool singular = false;
	// B_EDGE
	int trim_index[2] = {-1, -1};
	double trim_t[2] = {DBL_MAX, DBL_MAX};
	int edge_index = -1;
	double t = DBL_MAX;
	// B_SURF
	int face_index = -1;
	...
	// When the point is a surface point, we can store one normal vector
	ON_3dVector n_surf = ON_3dVector::UnsetVector;

	// For B_SURF point types, store per-face or singular normals
	std::unordered_map<int, ON_3dVector> fnorms;
};
