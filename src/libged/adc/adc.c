...
#define ADC_COMMIT_RETURN(_ret) \
	...
	if (argc == 1) {
		...
		adc_grid_to_adc_view(&adc, view2model, RT_VIEW_MAX);
	...
	if (argc == 1) {
		...
		adc_model_to_adc_view(&adc, model2view, RT_VIEW_MAX);
		adc_view_to_adc_grid(&adc, model2view);
	    }
	...
	if (argc == 0) {
	    adc_reset(&adc, view2model, model2view);
