...
rtcheck_vector_handler(void *clientData, int type)
{
	...
	//
	// The adapter clears both shared and local view scopes, matching the
	// legacy all-scope cleanup behavior without exposing BSG traversal here.
	const char *sname = "rtcheck::";
	struct bsg_view *view = (struct bsg_view *)ged_view_active_ctx(gedp);
	(void)ged_draw_view_features_remove_prefix(view, sname);
...
ged_rtcheck2_core(struct ged *gedp, int argc, const char *argv[])
{
	...
	*vp = 0;
    ...
    rtcp->csize = rt_view_scale_from_bsg(view) * 0.01;
