				  ...
				  const point_t p)
{
	...
	primitive_lod_line_set_append(realization, p, RT_PRIMITIVE_LINE_DRAW);
}
		...
		const struct bv_view_info *UNUSED(info))
{
	...
	for (size_t i = 0; i < realization.line_count; i++) {
		...
		case RT_PRIMITIVE_LINE_MOVE:
		    vlist_cmd = RT_VLIST_LINE_MOVE;
		    break;
		case RT_PRIMITIVE_LINE_DRAW:
		    vlist_cmd = RT_VLIST_LINE_DRAW;
		    break;
		case RT_PRIMITIVE_POINT_DRAW:
		    vlist_cmd = RT_VLIST_POINT_DRAW;
		    break;
		default:
		    primitive_lod_line_set_free(&realization);
		    return -1;
	    }
	    RT_ADD_VLIST(vlfree, vhead, realization.line_points[i],
		    vlist_cmd);
	}
    }
...
rt_extrude_tess(struct nmgregion **r, struct model *m, struct rt_db_internal *ip, const struct bg_tess_tol *ttol, const struct bn_tol *tol)
{
    ...
    /* count vertices */
    vert_count = 0;
    for (BU_LIST_FOR (vlp, bg_vlist, &vhead)) {
	for (i = 0; i < vlp->nused; i++) {
	    if (vlp->cmd[i] == RT_VLIST_LINE_DRAW)
		vert_count++;
	}
    }
    ...
    *r = nmg_mrsv(m);
	...
	for (BU_LIST_FOR (vlp, bg_vlist, &vhead)) {
		...
		if (vlp->cmd[j] == RT_VLIST_LINE_DRAW) {
