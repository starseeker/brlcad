...
/** @{ */
/** @file libbrlobol/evaluated_points.cpp
 *
...
static int
brlobol_evaluated_points_transform_face_set(
    struct rt_primitive_indexed_face_set *face_set,
    const mat_t matrix)
{
    if (!face_set || !matrix)
    ...
    struct rt_primitive_indexed_face_set *face_set)
{
    ...
    if (ret == BRLCAD_OK) {
	ret = brlobol_evaluated_points_transform_face_set(face_set,
		path_matrix);
	if (ret != BRLCAD_OK)
	    brlobol_evaluated_points_face_set_free(face_set);
    }
