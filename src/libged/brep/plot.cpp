...
static void
brep_plot_add(struct bg_line_layer *layer, const fastf_t *point, int command)
{
    point_t pt;
    ...
    VMOVE(pt, point);
    (void)bg_line_layer_add(layer, pt, command);
}
...
brep_plot_fast_cdt(struct bu_vls *vls,
		   ...
		   const ON_Brep *brep,
		   struct bg_line_layer_builder *plot,
		   int index,
		   int plottype,
		   int num_points)
{
    struct bg_line_layer *layer = bg_line_layer_builder_find(plot, YELLOW);
    return brep_facecdt_plot(vls, solid_name, ttol, tol, brep, layer, index, plottype, num_points);
}


static int
brep_plot_cdt_state(struct bg_line_layer_builder *plot,
		    int r,
		    int g,
		    int b,
		    int mode,
...
{
    struct bg_line_layer *layer = bg_line_layer_builder_find(plot, r, g, b);
    return ON_Brep_CDT_Plot(layer, mode, s_cdt);
}


void
plotpoint(const ON_3dPoint &point, struct bg_line_layer_builder *plot, const int red = 255, const int green = 255, const int blue = 0)
{
    struct bg_line_layer *vhead;
    vhead = bg_line_layer_builder_find(plot, red, green, blue);
    brep_plot_add(vhead, point, BG_GEOMETRY_POINT_DRAW);
    return;
}

void
plottrim(const ON_BrepTrim &trim, struct bg_line_layer_builder *plot, int plotres, bool dim3d, const int red = 255, const int green = 255, const int blue = 0)
{
    struct bg_line_layer *vhead;
	...
	if (k != 0) {
	    brep_plot_add(vhead, pt, BG_GEOMETRY_LINE_DRAW);
	} else {
	    brep_plot_add(vhead, pt, BG_GEOMETRY_LINE_MOVE);
	}
    }
	...
	VMOVE(pt2, p);
	brep_plot_add(vhead, pt1, BG_GEOMETRY_LINE_MOVE);
	brep_plot_add(vhead, pt2, BG_GEOMETRY_LINE_DRAW);
    }
#endif
...
void
plotcurve(const ON_Curve &curve, struct bg_line_layer_builder *plot, int plotres, const int red = 255, const int green = 255, const int blue = 0)
{
    struct bg_line_layer *vhead;
    fastf_t pt1[3], pt2[3];
...
void
plotsurface(const ON_Surface &surf, struct bg_line_layer_builder *plot, int isocurveres, int gridres, const int red = 200, const int green = 200, const int blue = 200)
{
    struct bg_line_layer *vhead = NULL;
    fastf_t pt1[3], pt2[3];
    fastf_t hsv[3];
	...
	if (u == 0 || u == gridres) {
	    ...
	    vhead = bg_line_layer_builder_find(plot, (int)(fill_rgb[0]), (int)(fill_rgb[1]), (int)(fill_rgb[2]));
	}
...
void
plotface(const ON_BrepFace &face, struct bg_line_layer_builder *plot, int plotres, bool dim3d, const int red = 255, const int green = 255, const int blue = 0)
{
    struct bg_line_layer *vhead;
...
static void
plotUVDomain2d(ON_BrepFace *face, struct bg_line_layer_builder *plot)
{
    struct bg_line_layer *vhead;
    const ON_Surface* surf = face->SurfaceOf();
    ...
    vhead = bg_line_layer_builder_find(plot, PURERED);
...
static void
drawisoUCheckForTrim(const SurfaceTree* st, struct bg_line_layer_builder *plot, fastf_t from, fastf_t to, fastf_t v, int UNUSED(curveres))
{
    struct bg_line_layer *vhead;
    fastf_t pt1[3], pt2[3];
    ...
    vhead = bg_line_layer_builder_find(plot, YELLOW);
...
static void
drawisoVCheckForTrim(const SurfaceTree* st, struct bg_line_layer_builder *plot, fastf_t from, fastf_t to, fastf_t u, int UNUSED(curveres))
{
    struct bg_line_layer *vhead;
    fastf_t pt1[3], pt2[3];
...
static void
drawisoU(const SurfaceTree* st, struct bg_line_layer_builder *plot, fastf_t from, fastf_t to, fastf_t v, int curveres)
{
    struct bg_line_layer *vhead;
    fastf_t pt1[3], pt2[3];
...
static void
drawisoV(const SurfaceTree* st, struct bg_line_layer_builder *plot, fastf_t from, fastf_t to, fastf_t u, int curveres)
{
    struct bg_line_layer *vhead;
    fastf_t pt1[3], pt2[3];
...
static void
drawBBNode(const SurfaceTree* st, struct bg_line_layer_builder *plot, const BBNode * node)
{
    if (node->isLeaf()) {
	//draw leaf
	if (node->m_trimmed) {
...
static void
plotFaceFromSurfaceTree(const SurfaceTree* st, struct bg_line_layer_builder *plot, int UNUSED(isocurveres), int UNUSED(gridres))
{
...
#define ARB_FACE(valp, a, b, c, d)			\
    brep_plot_add(vhead, valp[a], BG_GEOMETRY_LINE_MOVE);	\
    brep_plot_add(vhead, valp[b], BG_GEOMETRY_LINE_DRAW);	\
    brep_plot_add(vhead, valp[c], BG_GEOMETRY_LINE_DRAW);	\
    brep_plot_add(vhead, valp[d], BG_GEOMETRY_LINE_DRAW);
...
static unsigned int
plotsurfaceleafs(const SurfaceTree* surf, struct bg_line_layer_builder *plot, bool dim3d)
{
    struct bg_line_layer *vhead;
    ...
    vhead = bg_line_layer_builder_find(plot, PURERED);
    brep_plot_add(vhead, min, BG_GEOMETRY_LINE_MOVE);
    vhead = bg_line_layer_builder_find(plot, BLUE);
    brep_plot_add(vhead, min, BG_GEOMETRY_LINE_MOVE);
    vhead = bg_line_layer_builder_find(plot, MAGENTA);
    brep_plot_add(vhead, min, BG_GEOMETRY_LINE_MOVE);
...
static void
plottrimleafs(const SurfaceTree* st, struct bg_line_layer_builder *plot, bool dim3d)
{
    struct bg_line_layer *vhead;
	...
	const BRNode* bb = *i;
	if (bb->m_XIncreasing) {
	    vhead = bg_line_layer_builder_find(plot, GREEN);
	} else {
	    vhead = bg_line_layer_builder_find(plot, BLUE);
	}
	bb->GetBBox(min, max);
...
static void
plottrimdirection(const ON_BrepFace &face, struct bg_line_layer_builder *plot, int plotres)
{
    struct bg_line_layer *vhead;
    const ON_Surface* surf = face.SurfaceOf();
    fastf_t umin, umax;
...
static void
plotsurfacenormals(const ON_Surface &surf, struct bg_line_layer_builder *plot, int gridres)
{
    struct bg_line_layer *vhead;
    fastf_t pt1[3], pt2[3];
...
static void
plotsurfaceknots(ON_Surface &surf, struct bg_line_layer_builder *plot, bool dim3d)
{
    struct bg_line_layer *vhead;
    fastf_t pt1[3], pt2[3];
	...
	for (int u = 0; u <= spanu_cnt; u++) {
		...
		brep_plot_add(vhead, pt1, BG_GEOMETRY_POINT_DRAW);
	    }
	}
    }
    return;
...
static void
plot_nurbs_cv(struct bg_line_layer_builder *plot, int ucount, int vcount, const ON_NurbsSurface *ns)
{
    struct bg_line_layer *vhead;
    vhead = bg_line_layer_builder_find(plot, PEACH);
    ON_3dPoint cp;
    fastf_t pt1[3], pt2[3];
    int i, j, k, temp;
    for (k = 0; k < 2; k++) {
	...
	for (i = 0; i < ucount; ++i) {
		...
		brep_plot_add(vhead, pt2, BG_GEOMETRY_LINE_DRAW);
		VMOVE(pt1, cp);
		brep_plot_add(vhead, cp, BG_GEOMETRY_POINT_DRAW);
	    }
	}
	temp  = ucount;
	ucount = vcount;
...
void
_brep_plot_publish(struct ged *gedp, struct bg_line_layer_builder *plot, const char *sname)
{
    if (!gedp || !plot || !sname)
	return;
...
_brep_cmd_curve_2d_plot(void *bs, int argc, const char **argv)
{
    ...
    struct bg_line_layer_builder *plot = gib->gb->plot;
