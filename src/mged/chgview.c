...
mged_view_ring_save_current(struct mged_state *s, struct view_ring *vrp)
{
    ...
    rt_view_context_rotation_from_bsg(vrp->vr_rot_mat, view_state->vs_gvp);
    rt_view_context_center_from_bsg(vrp->vr_tvc_mat, view_state->vs_gvp);
...
mged_view_ring_restore_current(struct mged_state *s, struct view_ring *vrp)
{
    ...
    rt_view_context_rotation_set_bsg(view_state->vs_gvp, vrp->vr_rot_mat);
    MAT_DELTAS_GET_NEG(view_center, vrp->vr_tvc_mat);
    rt_view_context_center_vec_set_bsg(view_state->vs_gvp, view_center);
    rt_view_context_scale_set_bsg(view_state->vs_gvp, vrp->vr_scale);
}
                       ...
                       int *did_sca)
{
    ...
    switch (ke->type) {
        ...
        case KNOB_TRA_ABS: {
            int axis = ke->axis;
            fastf_t view_scale = rt_view_context_scale_from_bsg(view_state->vs_gvp);
            if (axis < 0) return BRLCAD_ERROR;
            fastf_t *abs_arr = NULL, *last_arr = NULL;
            switch (mged_variables->mv_coords) {
                case 'm':
	  ...
	  mat_t newrot)
{
	...
	case 'o':
	    ...
	    break;
	case 'v':
	    rt_view_context_rotation_from_bsg(view_rotation, view_state->vs_gvp);
	...
	switch (rotate_about) {
		...
		rt_view_context_view2model_from_bsg(view2model, view_state->vs_gvp);
	   ...
	   const vect_t tvec)
{
	...
	case 'v':
	default:
	    view_scale = rt_view_context_scale_from_bsg(view_state->vs_gvp);
	    rt_view_context_view2model_from_bsg(view2model, view_state->vs_gvp);
	    rt_view_context_center_from_bsg(view_center, view_state->vs_gvp);
...
cmd_size(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
    ...
    if (ret == BRLCAD_OK) {
	fastf_t view_scale = rt_view_context_scale_from_bsg(view_state->vs_gvp);
	fastf_t view_initial_scale = rt_view_context_initial_scale_from_bsg(view_state->vs_gvp);
	fastf_t view_absolute_scale = 1.0 - view_scale / view_initial_scale;

	if (view_absolute_scale < 0.0) {
	    view_absolute_scale /= 9.0;
	}
	rt_view_context_absolute_scale_set_bsg(view_state->vs_gvp, view_absolute_scale);
...
size_reset(struct mged_state *s)
{
    ...
    rt_view_context_initial_scale_set_bsg(view_state->vs_gvp,
	    rt_view_context_scale_from_bsg(view_state->vs_gvp));
    mged_refresh_request_view(s, view_state, RT_VIEW_REFRESH_VIEW_BSG);
}
...
f_status(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
	...
	rt_view_context_center_from_bsg(view_center, view_state->vs_gvp);
	rt_view_context_rotation_from_bsg(view_rotation, view_state->vs_gvp);
	rt_view_context_model2view_from_bsg(model2view, view_state->vs_gvp);
	rt_view_context_view2model_from_bsg(view2model, view_state->vs_gvp);
...
mged_print_knobvals(struct mged_state *s, Tcl_Interp *interp)
{
	...
	if (s->s_edit->es_edclass == EDIT_CLASS_SCALE && mged_variables->mv_transform == 'e') {
	    ...
	    bu_vls_printf(&vls, "aS = %f\n", rt_view_context_absolute_scale_from_bsg(view_state->vs_gvp));
	}
		...
		const char *token)
{
	...
	rt_view_context_knobs_reset_bsg(view_state->vs_gvp, RT_VIEW_KNOBS_ALL_BSG);
	if (MEDIT(s)) {
...
f_knob(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
	...
	if (token_is_core_view(ke)) {
		...
		if (using_librt_edit) {
		    ...
		    char save_coord = rt_view_context_coord_from_bsg(v);
		    rt_view_context_coord_set_bsg(v, mged_variables->mv_coords);
			...
			rt_view_context_coord_set_bsg(v, save_coord);
			goto usage;
		    }
		    rt_view_context_coord_set_bsg(v, save_coord);
		} else {
		...
		if (s->dbip) {
		    rt_view_context_unit_conversion_set_bsg(view_state->vs_gvp,
			    s->dbip->dbi_local2base,
			    s->dbip->dbi_base2local);
		}
		int model_mode = model_flag || (mged_variables->mv_coords == 'm' && !view_flag);
		if (rt_view_context_knobs_cmd_process_bsg(&view_rvec, &view_do_rot, &view_tvec, &view_do_tran,
			    view_state->vs_gvp,
			    token, fval,
	...
	if (view_do_tran) {
	    rt_view_context_knobs_translate_bsg(view_state->vs_gvp, view_tvec, model_mode_final);
	}

	if (view_do_rot) {
	    rt_view_context_knobs_rotate_bsg(view_state->vs_gvp, view_rvec, origin, vcoords,
...
mged_zoom(struct mged_state *s, double val)
{
    ...
    if (active_view &&
	rt_view_context_lod_policy_from_bsg(&lod_policy, active_view) &&
...
mged_svbase(struct mged_state *s)
{
    ...
    rt_view_context_initial_scale_set_bsg(view_state->vs_gvp, view_scale);
	...
	rt_view_context_knobs_state_set_bsg(view_state->vs_gvp, &view_state->k);
	rt_view_context_knobs_reset_bsg(view_state->vs_gvp, RT_VIEW_KNOBS_ABS_BSG);
	rt_view_context_knobs_state_from_bsg(&view_state->k, view_state->vs_gvp);
    }
    ...
    rt_view_context_absolute_scale_set_bsg(view_state->vs_gvp, 0.0);
...
cmd_mrot(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
	...
	return mged_erot(s, rt_view_context_coord_from_bsg(view_state->vs_gvp),
		rt_view_context_rotate_about_from_bsg(view_state->vs_gvp), rmat);
    } else {
	int ret;
...
cmd_rot(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
	...
	return mged_erot(s, coord, rt_view_context_rotate_about_from_bsg(view_state->vs_gvp),
		rmat);
    } else {
	int ret;
...
mged_vscale(struct mged_state *s, fastf_t sfactor)
{
    ...
    view_scale = rt_view_context_scale_from_bsg(view_state->vs_gvp) * sfactor;

    if (view_scale < RT_VIEW_MIN_SIZE) {
	view_scale = RT_VIEW_MIN_SIZE;
    }

    rt_view_context_scale_set_bsg(view_state->vs_gvp, view_scale);
    f = view_scale / rt_view_context_initial_scale_from_bsg(view_state->vs_gvp);

    if (f >= 1.0) {
	rt_view_context_absolute_scale_set_bsg(view_state->vs_gvp, (f - 1.0) / -9.0);
    } else {
	rt_view_context_absolute_scale_set_bsg(view_state->vs_gvp, 1.0 - f);
    }
...
cmd_sca(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
	...
	f = rt_view_context_scale_from_bsg(view_state->vs_gvp) /
	    rt_view_context_initial_scale_from_bsg(view_state->vs_gvp);
