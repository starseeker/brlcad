...
{
    icv_image_t *img = icv_image_create_with_channels(w, h, ICV_COLOR_SPACE_RGB, 4);
    for (size_t y = 0; y < h; y++) {
	for (size_t x = 0; x < w; x++) {
...
test_create_zero_and_pixel_io(void)
{
    ...
    rgba = icv_image_create_with_channels(2, 3, ICV_COLOR_SPACE_RGB, 4);
    CHECK(rgba != NULL && rgba->channels == 4 && rgba->alpha_channel == 1,
	  "icv_image_create_with_channels creates an RGBA image");
    CHECK(icv_image_create_with_channels(1, 1, ICV_COLOR_SPACE_RGB, 2) == NULL,
	  "icv_image_create_with_channels rejects an invalid RGB layout");
    icv_destroy(rgba);
