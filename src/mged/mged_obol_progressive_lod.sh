...
FRAME0="${WORKDIR}/mged_obol_progressive_lod_0.png"
...
for img in "$FRAME0" "$FRAME1" "$FRAME2" "$FRAME3"; do
    if [ ! -s "$img" ]; then
	echo "MGED Obol progressive LoD did not create PNG: $img" 1>&2
	cat "$LOG" 1>&2
	exit 1
    fi
done
...
if ! grep -Eq 'last_visited_meshes: [1-9][0-9]*' "$LOG"; then
    echo "MGED Obol progressive LoD did not visit mesh LoD candidates" 1>&2
    cat "$LOG" 1>&2
    exit 1
fi

if ! grep -Eq 'last_submitted_tasks: [1-9][0-9]*' "$LOG"; then
    echo "MGED Obol progressive LoD did not submit LoD tasks" 1>&2
    cat "$LOG" 1>&2
    exit 1
fi

if ! grep -Eq 'last_applied_results: [1-9][0-9]*' "$LOG"; then
    echo "MGED Obol progressive LoD did not apply LoD results" 1>&2
    cat "$LOG" 1>&2
    exit 1
fi
...
if ! grep -Eq 'active_lod_aabb_proxies: [1-9][0-9]*' "$LOG"; then
    echo "MGED Obol progressive LoD did not activate an AABB proxy payload" 1>&2
    cat "$LOG" 1>&2
    exit 1
fi
...
if ! grep -Eq 'active_lod_mesh_payloads: [1-9][0-9]*' "$LOG"; then
    echo "MGED Obol progressive LoD did not activate a mesh LoD payload" 1>&2
    cat "$LOG" 1>&2
    exit 1
fi
...
if diff01 <= 0 or diff12 <= 0 or diff23 <= 0 or diff03 <= 0:
