...
static void
labelface_export_record(const SoBRLExportAction::ObjectRecord &rec,
			struct labelface_data *lfd)
{
    if (!lfd || !rec.databaseIntent ||
	!labelface_record_matches(lfd->dbip, rec.path.getString(), lfd->dp))
	return;
	...
	BObolLabel item;
	item.text = label;
	item.point = SbVec3f(static_cast<float>(avg_pt[X]),
	    static_cast<float>(avg_pt[Y]), static_cast<float>(avg_pt[Z]));
	item.hasColor = TRUE;
	item.color = SbColor(1.0f, 1.0f, 1.0f);
	lfd->labels.push_back(item);
    }
}
...
ged_labelface_core(struct ged *gedp, int argc, const char *argv[])
{
	...
	bu_vls_printf(gedp->ged_result_str,
	    "labelface: current view has no BObol display endpoint");
	return BRLCAD_ERROR;
    }
	...
	for (const SoBRLExportAction::ObjectRecord &record : records)
	    labelface_export_record(record, &lfd);
    }

    BObolViewController *controller =
	ged_bobol_shared_view_controller(gedp);
    BObolFeatureHandle existing = controller ?
	controller->features().find(LABELFACE_FEATURE_NAME,
	    BOBOL_FEATURE_SCOPE_SHARED) : BObolFeatureHandle();
    if (controller && existing.isValid())
	(void)controller->features().remove(existing);
    const bool published = controller && (lfd.labels.empty() ||
	controller->features().publishLabels(LABELFACE_FEATURE_NAME,
	    BObolFeatureScope::Shared, lfd.labels).isValid());
    if (!published) {
...
extern "C" {
int ged_nmg_cmface_core(struct ged *gedp, int argc, const char *argv[]);
int ged_nmg_collapse_core(struct ged *gedp, int argc, const char *argv[]);
int ged_nmg_fix_normals_core(struct ged *gedp, int argc, const char *argv[]);
int ged_nmg_kill_f_core(struct ged* gedp, int argc, const char* argv[]);
int ged_nmg_kill_v_core(struct ged* gedp, int argc, const char* argv[]);
int ged_nmg_make_v_core(struct ged *gedp, int argc, const char *argv[]);
int ged_nmg_mm_core(struct ged *gedp, int argc, const char *argv[]);
int ged_nmg_move_v_core(struct ged* gedp, int argc, const char* argv[]);
int ged_nmg_simplify_core(struct ged *gedp, int argc, const char *argv[]);
}
